...
for K_ind, K_vec in enumerate(K_cluster):
    ...
    for f_ind, filename in enumerate(filenames):
        ...
        G_K_w = data['functions/cluster_greens_function_G_k_w/data']

        plt.plot(wn[len(wn)/2-wn_cutoff:len(wn)/2+wn_cutoff], G_K_w[len(wn)/2-wn_cutoff:len(wn)/2+wn_cutoff, K_ind, s2, b2, s1, b1, 0],
                 marker=markers[f_ind], color=colors[f_ind], label=filename)

    # plt.xlabel('$\omega_n$')
    plt.ylabel(r'$\mathrm{Re} \, G(\vec{K}, i \omega_n)$')
    ...
    for f_ind, filename in enumerate(filenames):
        ...
        plt.plot(wn[len(wn)/2-wn_cutoff:len(wn)/2+wn_cutoff], G_K_w[len(wn)/2-wn_cutoff:len(wn)/2+wn_cutoff, K_ind, s2, b2, s1, b1, 1],
                 marker=markers[f_ind], color=colors[f_ind], label=filename)
        
    plt.xlabel('$\omega_n$')
    plt.ylabel(r'$\mathrm{Im} \, G(\vec{K}, i \omega_n)$')
    plt.legend(loc='upper right')
    plt.savefig('sp-greens-function_K='+str(K_ind)+'.pdf')
