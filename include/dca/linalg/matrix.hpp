...
template <typename ScalarType, DeviceType device_name,
          class ALLOC = util::DefaultAllocator<ScalarType, device_name>>
...
public:
  ...
  template <DeviceType rhs_device_name, class rhs_ALLOC>
  void setAsync(const Matrix<ScalarType, rhs_device_name, rhs_ALLOC>& rhs,
...
#ifndef NDEBUG
    // This makes debugging matrix issues easier since there is never garbage in expanded matrices
    if constexpr (device_name == linalg::DeviceType::CPU)
      std::fill_n(new_data, nrElements(new_capacity), 0);
#endif
...
#ifndef NDEBUG
  else if (new_size.first > size_.first || new_size.second > size_.second) {
    // we need to cover case where we expand the matrix but no allocation is necessary.
...
void Matrix<ScalarType, device_name, ALLOC>::setAsync(
    const Matrix<ScalarType, rhs_device_name, rhs_ALLOC>& rhs, const util::GpuStream& stream) {
  resizeNoCopy(rhs.size_);
