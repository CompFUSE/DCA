...
int packing_interface<MPI_LIBRARY>::get_buffer_size(scalar_type /*item*/) {
  ...
  MPI_Pack_size(type_map_interface<MPI_LIBRARY, scalar_type>::factor(),
                type_map_interface<MPI_LIBRARY, scalar_type>::value(), grouping.get(), &size);
...
int packing_interface<MPI_LIBRARY>::get_buffer_size(std::basic_string<scalar_type> str) {
  ...
  */
  ...
  {
    int size(0);
    MPI_Pack_size(count, type_map_interface<MPI_LIBRARY, scalar_type>::value(), grouping.get(),
                  &size);
...
template <typename object_type>
void packing_interface<MPI_LIBRARY>::pack_or_unpack(bool packing, int* buffer, int size,
                                                    int& off_set, object_type& item) {
  if (packing)
...
void packing_interface<MPI_LIBRARY>::unpack(int* buffer, int size, int& off_set, scalar_type& item) {
  ...
  MPI_Unpack(buffer, size, &off_set, &tmp, type_map_interface<MPI_LIBRARY, scalar_type>::factor(),
             type_map_interface<MPI_LIBRARY, scalar_type>::value(), grouping.get());

  item = tmp;
}

template <typename scalar_type>
void packing_interface<MPI_LIBRARY>::unpack(int* buffer, int size, int& off_set,
