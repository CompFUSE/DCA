...
void nfft_time_domain<NAME, dnfft_type>::initialize(dnfft_type& dnfft_obj) {
  if (not is_initialized()) {
    ...
    const int OVER_SAMPLING = dnfft_obj.getOversamplingFactor();
    const int MAX_FREQUENCY = dnfft_obj.getMaximumFrequency();
    const int WINDOW_SAMPLING = dnfft_obj.getWindowSamplingFactor();
