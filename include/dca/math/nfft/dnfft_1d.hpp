...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling = 8, NfftModeNames mode = CUBIC>
class Dnfft1D {
public:
  ...
  void accumulate(int linind, Real t_val, Scalar f_val);
  // Version with subindices.
  // subind contains the subindices of the sample w.r.t. the subdomains of p_dmn.
  void accumulate(const int* subind, Real t_val, Scalar f_val);
  ...
  using FTau = func::function<Scalar, PaddedTimePDmn>;

  /** Added for debugging */
  friend constexpr bool operator==(const Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>& dfft_a,
                                   const Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>& dfft_b) {
...
private:
  ...
  void convoluteToFTauExact(int index, Real t_val, Scalar f_val);
  void convoluteToFTauFineLinearInterpolation(int index, Real t_val, Scalar f_val);
  void convoluteToFTauFineCubicInterpolation(int index, Real t_val, Scalar f_val);
  ...
  template <class OutReal>
  void transformFTauToFW(func::function<std::complex<OutReal>, func::dmn_variadic<WDmn, PDmn>>& f_w,
                         std::true_type) const;
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::resetAccumulation() {
  f_tau_ = 0.;
}

template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::initializeDomains(const ThisType& this_obj) {
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::initializeStaticFunctions() {
  ...
  if (mode == LINEAR || mode == CUBIC) {
    ...
    Real delta = convolution_time_values(0, 1) - convolution_time_values(0, 0);

    for (int i = 0; i < OversamplingDmn::dmn_size(); ++i) {
      for (int j = 0; j < WindowSamplingDmn::dmn_size(); ++j) {
        assert(std::abs(convolution_time_values(i, j) - fineTau(index)) < 1.e-6);

        Real tau = convolution_time_values(i, j);

        Real f0 = WindowFunction::phi_t(tau);
        Real f1 = WindowFunction::phi_t(tau + delta);

        Real df0 = WindowFunction::d_phi_t(tau);
        Real df1 = WindowFunction::d_phi_t(tau + delta);

        Real a = f0;
        Real b = df0;

        Real c = -(3. * f0 - 3. * f1 + 2. * df0 * delta + df1 * delta) / std::pow(delta, 2);
        Real d = -(-2. * f0 + 2. * f1 - 1. * df0 * delta - df1 * delta) / std::pow(delta, 3);
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
inline void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::accumulate(const int linind,
                                                                        const Real t_val,
                                                                        const Scalar f_val) {
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
inline void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::accumulate(const int* const subind,
                                                                        const Real t_val,
                                                                        const Scalar f_val) {
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
template <typename OtherScalar>
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::finalize(
    func::function<std::complex<OtherScalar>, func::dmn_variadic<WDmn, PDmn>>& f_w) {
  foldTimeDomainBack();
  transformFTauToFW(f_w);
}

template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::convoluteToFTauExact(const int index,
                                                                           const Real t_val,
                                                                           const Scalar f_val) {
  assert(t_val > -0.5 - 1.e-6 && t_val < 0.5 + 1.e-6);

  const Real T_0 = PaddedTimeDmn::parameter_type::first_element();
  const Real one_div_Delta = PaddedTimeDmn::parameter_type::get_one_div_Delta();
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
inline void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::convoluteToFTauFineLinearInterpolation(
    const int index, const Real t_val, const Scalar f_val) {
  assert(t_val > -0.5 - 1.e-6 && t_val < 0.5 + 1.e-6);

  const auto t_0 = WindowFunctionTimeDmn::parameter_type::first_element();
  const auto T_0 = PaddedTimeDmn::parameter_type::first_element();

  const auto one_div_delta = PaddedTimeDmn::parameter_type::get_one_div_delta();
  const auto one_div_Delta = PaddedTimeDmn::parameter_type::get_one_div_Delta();
  ...
  auto diff_tau = tau(tau_0) - t_val - fineTau(tau_1);
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
inline void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::convoluteToFTauFineCubicInterpolation(
    const int index, const Real t_val, const Scalar f_val) {
  assert(t_val > -0.5 - 1.e-6 && t_val < 0.5 + 1.e-6);
  ...
  const auto delta = PaddedTimeDmn::parameter_type::get_delta();
  const auto Delta = PaddedTimeDmn::parameter_type::get_Delta();

  const auto one_div_delta = PaddedTimeDmn::parameter_type::get_one_div_delta();
  const auto one_div_Delta = PaddedTimeDmn::parameter_type::get_one_div_Delta();

  const int tau_0 = (t_val - T_0) * one_div_Delta;
  const auto t0_val_lb = T_0 + tau_0 * Delta;

  assert(tau(tau_0) - 1.e-6 < t_val && t_val < tau(tau_0 + 1) + 1.e-6);
  assert(std::abs(tau(tau_0) - t0_val_lb) < 1.e-6);

  // int tau_1 = (tau(tau_0)-t_val-t_0)*one_div_delta;
  const int tau_1 = (t0_val_lb - t_val - t_0) * one_div_delta;
  const auto t1_val_lb = t_0 + tau_1 * delta;

  const auto diff_tau = t0_val_lb - t_val - t1_val_lb;  // fineTau(tau_1);
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::foldTimeDomainBack() {
...
void Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::transformFTauToFW(
...

  {
    ...
    plan = fftw_plan_dft_r2c_1d(n, f_in.data(), reinterpret_cast<fftw_complex*>(f_out.data()),
                                FFTW_ESTIMATE);
  }

  func::function<std::complex<Scalar>, LeftOrientedPDmn> f_omega;
  for (int p_ind = 0; p_ind < PDmn::dmn_size(); ++p_ind) {
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>& Dnfft1D<Scalar, WDmn, PDmn, oversampling,
...
template <typename Scalar, typename WDmn, typename PDmn, int oversampling, NfftModeNames mode>
auto& Dnfft1D<Scalar, WDmn, PDmn, oversampling, mode>::get_convolution_time_values() {
  static func::function<Real, func::dmn_variadic<OversamplingDmn, WindowSamplingDmn>>
