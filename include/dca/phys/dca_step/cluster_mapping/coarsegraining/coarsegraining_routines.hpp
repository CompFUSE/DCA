...
void coarsegraining_routines<parameters_type, K_dmn>::compute_G_q_w(
  ...
  if (nr_threads == 1)
    quadrature_integration<typename parameters_type::ThreadingType,
                           q_dmn_t>::quadrature_integration_G_q_w_st(I_q, H_q, S_q, G_q);
  else
    quadrature_integration<typename parameters_type::ThreadingType,
                           q_dmn_t>::quadrature_integration_G_q_w_mt(nr_threads, I_q, H_q, S_q, G_q);
}
...
void coarsegraining_routines<parameters_type, K_dmn>::compute_G_q_t(
  ...
  if (nr_threads == 1)
    quadrature_integration<typename parameters_type::ThreadingType, q_dmn_t>::quadrature_integration_G_q_t_st(
        beta, f_val, t_val, I_q, H_q, G_q);
  else
    quadrature_integration<typename parameters_type::ThreadingType, q_dmn_t>::quadrature_integration_G_q_t_mt(
