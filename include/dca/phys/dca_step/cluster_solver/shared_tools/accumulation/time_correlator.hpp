...
private:
  ...
  void computeG0(linalg::Matrix<Scalar, GPU>& G0_mat, const Config<GPU>& config_l,
                 const Config<GPU>& config_r);

  //  template <typename = std::enable_if_t<device == CPU>>
  void computeG0(linalg::Matrix<Scalar, CPU>& G0_mat, const Config<CPU>& config_l,
  ...
  linalg::Matrix<Scalar, device> G0_l_;
  linalg::Matrix<Scalar, device> G0_r_;
  linalg::Matrix<Scalar, device> G0_M_;
  std::array<linalg::Matrix<Scalar, device>, 2> G_;

  std::array<linalg::Matrix<Scalar, CPU>, 2> G_host_;

  std::unique_ptr<Config<device>> fixed_config_;
  ...
  std::array<math::statistics::Autocorrelation<Scalar>, n_correlators_> autocorrelations_;
};

template <class Parameters, typename Scalar, DeviceType device>
TimeCorrelator<Parameters, Scalar, device>::TimeCorrelator(const Parameters& parameters_ref, int id, G0Interpolation<device, Scalar>& g0)
  ...
  // \todo make fixed_configuration_ in mainthread scope and pass a reference.
...
template <class Parameters, typename Scalar, DeviceType device>
void TimeCorrelator<Parameters, Scalar, device>::initializeFixedConfiguration() {
  fixed_config_ = std::make_unique<Config<device>>(n_bands_);

  Config<CPU> host_config(n_bands_);
  double* tau = host_config.template get<0>();
...
template <class Parameters, typename Scalar, DeviceType device>
float TimeCorrelator<Parameters, Scalar, device>::get_FLOPs() const {
  return flops_;
}

template <class Parameters, typename Scalar, DeviceType device>
auto& TimeCorrelator<Parameters, Scalar, device>::operator+=(
    const TimeCorrelator<Parameters, Scalar, device>& rhs) {
...
template <class Parameters, typename Scalar, DeviceType device>
void TimeCorrelator<Parameters, Scalar, device>::sumConcurrency(const Concurrency& concurrency) {
  // TODO: delay.
  for (int i = 0; i < autocorrelations_.size(); ++i) {
...
template <class Parameters, typename Scalar, DeviceType device>
void TimeCorrelator<Parameters, Scalar, device>::reset() {
