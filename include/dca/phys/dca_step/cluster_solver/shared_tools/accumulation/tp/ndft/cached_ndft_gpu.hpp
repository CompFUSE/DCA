...
public:
  ...
  void execute(const Configuration& configuration, const linalg::Matrix<double, linalg::GPU>& M,
               Matrix& M_r_r_w_w);
...
void CachedNdft<Real, RDmn, WDmn, WPosDmn, linalg::GPU, non_density_density>::execute(
    const Configuration& configuration, const linalg::Matrix<double, linalg::GPU>& M, Matrix& M_out) {
