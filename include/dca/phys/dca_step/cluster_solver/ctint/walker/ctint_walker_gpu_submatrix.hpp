...
void CtintWalkerSubmatrixGpu<Parameters, DIST>::synchronize() const {
  ...
  checkRC(cudaStreamSynchronize(*get_stream()));
}
...
void CtintWalkerSubmatrixGpu<Parameters, DIST>::uploadConfiguration() {
  ...
  for (int s = 0; s < 2; ++s) {
    ...
    f_dev_[s].setAsync(values, *get_stream());
  }
#ifdef DEBUG_SUBMATRIX
...
void CtintWalkerSubmatrixGpu<Parameters, DIST>::updateM() {
  ...
  for (int s = 0; s < 2; ++s)
    Gamma_inv_dev_[s].setAsync(Gamma_inv_[s], *get_stream());
  ...
  for (int s = 0; s < 2; ++s) {
    ...
    gamma_index_dev_[s].setAsync(gamma_index, *get_stream());
  }
  ...
  for (int s = 0; s < 2; ++s) {
    ...
    move_indices_dev_[s].setAsync(move_indices_[s], *get_stream());
    // Note: an event synchronization might be necessary if the order of operation is changed.
    linalg::matrixop::copyCols(G_dev_[s], move_indices_dev_[s], old_G, thread_id_, 0);
    linalg::matrixop::copyRows(M_dev_[s], move_indices_dev_[s], old_M, thread_id_, 0);

    auto& tmp = G_dev_[s];
    // Note: the following resize is safe as it does not deallocate.
    tmp.resizeNoCopy(std::make_pair(gamma_size, n_max_[s]));
    linalg::matrixop::gemm(Gamma_inv_dev_[s], old_M, tmp, thread_id_, 0);
    linalg::matrixop::gemm(Scalar(-1.), old_G, tmp, Scalar(1.), M_dev_[s], thread_id_, 0);
  ...
  for (int s = 0; s < 2; ++s) {
    removal_list_dev_[s].setAsync(removal_list_[s], *get_stream());
    source_list_dev_[s].setAsync(source_list_[s], *get_stream());
...
void CtintWalkerSubmatrixGpu<Parameters, DIST>::computeM(
  ...
  for (int s = 0; s < 2; ++s) {
    ...
    details::multiplyByInverseFFactor(m_in, m_out, f_dev_[s].ptr(), *get_stream());
