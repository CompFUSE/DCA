...
template <typename Concurrency>
int EqualTimeParameters::getBufferSize(Concurrency& concurrency) const {
  int buffer_size = 0;
...
template <typename Concurrency>
void EqualTimeParameters::pack(Concurrency& concurrency, int* buffer, int buffer_size,
                               int& position) const {
  concurrency.pack(buffer, buffer_size, position, do_equal_time_measurements_);
}

template <typename Concurrency>
void EqualTimeParameters::unpack(Concurrency& concurrency, int* buffer, int buffer_size,
                                 int& position) {
