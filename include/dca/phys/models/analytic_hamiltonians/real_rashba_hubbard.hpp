...
#ifndef DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_REAL_RASHBA_HUBBARD_HPP
#define DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_REAL_RASHBA_HUBBARD_HPP
...
template <typename PointGroup>
const double* RealRashbaHubbard<PointGroup>::initializeRDCABasis() {
  static const std::array<double, 4> r_base{1, 0, 0, 1};
  return r_base.data();
}

template <typename PointGroup>
const double* RealRashbaHubbard<PointGroup>::initializeRLDABasis() {
  static const std::array<double, 4> r_base{1, 0, 0, 1};
  return r_base.data();
}

template <typename PointGroup>
std::vector<int> RealRashbaHubbard<PointGroup>::flavors() {
...
template <typename PointGroup>
std::vector<std::vector<double>> RealRashbaHubbard<PointGroup>::aVectors() {
  return {{0, 0}, {0, 0}};
}

template <typename PointGroup>
std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> RealRashbaHubbard<
...
template <typename PointGroup>
template <typename BandDmn, typename SpinDmn, typename RDmn, typename parameters_type>
void RealRashbaHubbard<PointGroup>::initializeHInteraction(
    func::function<typename parameters_type::Real,
                   func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                      func::dmn_variadic<BandDmn, SpinDmn>, RDmn>>& H_interaction,
    const parameters_type& parameters) {
...
template <typename PointGroup>
template <class domain>
void RealRashbaHubbard<PointGroup>::initializeHSymmetry(func::function<int, domain>& H_symmetries) {
  H_symmetries = -1;
...
template <typename PointGroup>
template <typename ParametersType, typename ScalarType, typename BandDmn, typename SpinDmn, typename KDmn>
void RealRashbaHubbard<PointGroup>::initializeH0(
    const ParametersType& parameters,
    func::function<ScalarType, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                                  func::dmn_variadic<BandDmn, SpinDmn>, KDmn>>& H_0) {
  if (BandDmn::dmn_size() != BANDS)
    throw std::logic_error("Rashba lattice has two bands.");
  // if (SpinDmn::dmn_size() != 2)
  //   throw std::logic_error("Spin domain size must be 2.");
...
template <typename PointGroup>
template <typename FNEW, typename KVECS>
void RealRashbaHubbard<PointGroup>::timeOrFrequencySymmetrySpecial(
...
template <typename PointGroup>
void RealRashbaHubbard<PointGroup>::clusterSymmetrySpecial(int b0, int b1, int k_ind, int& k_new,
...
// void RealRashbaHubbard<PointGroup>::H0UpDownFiniteK(int k) {
...
#endif  // DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_REAL_RASHBA_HUBBARD_HPP
