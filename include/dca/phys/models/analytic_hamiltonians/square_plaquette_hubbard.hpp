...
#ifndef DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_SQUARE_PLAQUETTE_HUBBARD_HPP
#define DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_SQUARE_PLAQUETTE_HUBBARD_HPP
...
int squarePlaquetteHubbard<PointGroupType>::transformationSignOfR(int b1 [[maybe_unused]],
                                                         int b2 [[maybe_unused]],
                                                         int s [[maybe_unused]]) {
  return 1;  // TODO: FIXME
}
...
template <typename PointGroupType>
int squarePlaquetteHubbard<PointGroupType>::transformationSignOfK(int b1 [[maybe_unused]],
                                                         int b2 [[maybe_unused]],
                                                         int s [[maybe_unused]]) {
  return 1;  // TODO: FIXME
}
...
template <typename PointGroupType>
const double* squarePlaquetteHubbard<PointGroupType>::initializeRDCABasis() {
  static std::array<double, 4> basis{2,0,0,2};
  return basis.data();
}

template <typename PointGroupType>
const double* squarePlaquetteHubbard<PointGroupType>::initializeRLDABasis() {
  static std::array<double, 4> basis{2,0,0,2};
  return basis.data();
}
...
template <typename PointGroupType>
std::vector<std::vector<double>> squarePlaquetteHubbard<PointGroupType>::aVectors() {
  return {{0, 0}, {1, 0}, {0, 1}, {1, 1}};
}

template <typename PointGroupType>
std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> squarePlaquetteHubbard<
...
template <typename PointGroupType>
template <typename BandDmn, typename SpinDmn, typename RDmn, typename parameters_type>
void squarePlaquetteHubbard<PointGroupType>::initializeHInteraction(
    func::function<double, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                              func::dmn_variadic<BandDmn, SpinDmn>, RDmn>>& H_interaction,
    const parameters_type& parameters) {
...
template <typename PointGroupType>
template <class domain>
void squarePlaquetteHubbard<PointGroupType>::initializeHSymmetry(func::function<int, domain>& H_symmetries) {
  H_symmetries = -1;
}

template <typename PointGroupType>
template <typename ParametersType, typename ScalarType, typename BandDmn, typename SpinDmn, typename KDmn>
void squarePlaquetteHubbard<PointGroupType>::initializeH0(
    const ParametersType& parameters,
    func::function<ScalarType, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                                  func::dmn_variadic<BandDmn, SpinDmn>, KDmn>>& H_0) {
  if (BandDmn::dmn_size() != BANDS)
    throw std::logic_error("Plaquette Hubbard model has four bands.");
  if (SpinDmn::dmn_size() != 2)
    throw std::logic_error("Spin domain size must be 2.");
  ...
  for (int k_ind = 0; k_ind < KDmn::dmn_size(); ++k_ind) {
    ...
    for (int s = 0; s < 2; s++) {
      H_0(0, s, 1, s, k_ind) = -2. * t * std::cos(k[0]);
      H_0(0, s, 2, s, k_ind) = -2. * t * std::cos(k[1]);
      H_0(2, s, 3, s, k_ind) = -2. * t * std::cos(k[0]);
      H_0(1, s, 3, s, k_ind) = -2. * t * std::cos(k[1]);

      H_0(1, s, 0, s, k_ind) = -2. * t * std::cos(k[0]);
      H_0(2, s, 0, s, k_ind) = -2. * t * std::cos(k[1]);
      H_0(3, s, 2, s, k_ind) = -2. * t * std::cos(k[0]);
      H_0(3, s, 1, s, k_ind) = -2. * t * std::cos(k[1]);
    }
  }
}
...
#endif  // DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_SQUARE_PLAQUETTE_HUBBARD_HPP
