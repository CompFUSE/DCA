...
#ifndef DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_TWOORBITAL_HPP
#define DCA_PHYS_MODELS_ANALYTIC_HAMILTONIANS_TWOORBITAL_HPP
...
template <typename point_group_type>
double* twoOrbital<point_group_type>::initializeRDCABasis() {
...
template <typename point_group_type>
double* twoOrbital<point_group_type>::initializeKDCABasis() {
...
template <typename point_group_type>
double* twoOrbital<point_group_type>::initializeRLDABasis() {
...
template <typename point_group_type>
double* twoOrbital<point_group_type>::initializeKLDABasis() {
...
template <typename point_group_type>
std::vector<int> twoOrbital<point_group_type>::flavors() {
...
template <typename point_group_type>
std::vector<std::vector<double>> twoOrbital<point_group_type>::aVectors() {
...
template <typename point_group_type>
std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> twoOrbital<
...
template <typename point_group_type>
template <typename BandDmn, typename SpinDmn, typename RDmn, typename parameters_type>
void twoOrbital<point_group_type>::initializeHInteraction(
    func::function<double, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                              func::dmn_variadic<BandDmn, SpinDmn>, RDmn>>& H_interaction,
    const parameters_type& parameters) {
...
template <typename point_group_type>
template <class domain>
void twoOrbital<point_group_type>::initializeHSymmetry(func::function<int, domain>& H_symmetries) {
  H_symmetries = -1;
...
template <typename point_group_type>
template <typename ParametersType, typename ScalarType, typename BandDmn, typename SpinDmn, typename KDmn>
void twoOrbital<point_group_type>::initializeH0(
    const ParametersType& parameters,
    func::function<ScalarType, func::dmn_variadic<func::dmn_variadic<BandDmn, SpinDmn>,
                                                  func::dmn_variadic<BandDmn, SpinDmn>, KDmn>>& H_0) {
  if (BandDmn::dmn_size() != BANDS)
  ...
  for (int k_ind = 0; k_ind < KDmn::dmn_size(); ++k_ind) {
    ...
    const auto val12 = -4. * t12 * std::sin(k[0]) * std::sin(k[1]);
