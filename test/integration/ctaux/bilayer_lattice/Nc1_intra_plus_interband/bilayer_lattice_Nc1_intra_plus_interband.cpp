...
TEST(bilayerLattice_Nc1_intra_plus_interband, Self_Energy) {
  ...
  if (dca_test_env->concurrency.id() == dca_test_env->concurrency.first()) {
    ...
    reader.open_file(
        DCA_SOURCE_DIR
        "/test/integration/ctaux/bilayer_lattice/Nc1_intra_plus_interband/data.ED.hdf5");
  ...
  if (dca_test_env->concurrency.id() == dca_test_env->concurrency.first()) {
    ...
    reader.open_file(
        DCA_SOURCE_DIR
        "/test/integration/ctaux/bilayer_lattice/Nc1_intra_plus_interband/check_data.QMC.hdf5");
    reader.open_group("functions");
        ...
        for (int nu_ind_2 = 0; nu_ind_2 < nu::dmn_size(); ++nu_ind_2) {
          for (int nu_ind_1 = 0; nu_ind_1 < nu::dmn_size(); ++nu_ind_1) {
            EXPECT_NEAR(Sigma_QMC_check(nu_ind_1, nu_ind_2, k_ind, w_ind).real(),
                        Sigma_QMC(nu_ind_1, nu_ind_2, k_ind, w_ind).real(), 1.e-11);
            EXPECT_NEAR(Sigma_QMC_check(nu_ind_1, nu_ind_2, k_ind, w_ind).imag(),
                        Sigma_QMC(nu_ind_1, nu_ind_2, k_ind, w_ind).imag(), 1.e-11);
          }
        }
      }
    }
...
int main(int argc, char** argv) {
  ...
  dca_test_env = new dca::testing::DcaMpiTestEnvironment(
      ...
      "/test/integration/ctaux/bilayer_lattice/Nc1_intra_plus_interband/"
