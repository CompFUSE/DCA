...
#include "test/integration/statistical_tests/FeAs/fe_as_setup.hpp"

dca::testing::DcaMpiTestEnvironment* dca_test_env;

TEST(CtintFeAsValidationTest, GreensFunction) {
  //  dca::linalg::util::initializeMagma();

  using namespace dca::testing;
  const std::string ed_data_name = dca::testing::test_directory + "/ed_check.hdf5";
  ...
  if (id == dca_test_env->concurrency.first()) {
    ...
    test.printInfo("FeAsCu_testinfo.out", true);
  ...
  if (id == 0) {
    ...
    writer.open_file("FeAs_results.hdf5");
