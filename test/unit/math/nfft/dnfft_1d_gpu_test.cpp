...
using Dnfft1DGpuCmplxTest =
    dca::testing::SingleSectorAccumulationTest<std::complex<double>, n_bands, n_sites, n_frequencies>;
...
void computeWithCpuDnfft(dca::linalg::Matrix<Scalar, dca::linalg::CPU>& M, Configuration& config,
                         DnfftType& dnfft_obj,
...
TEST_F(Dnfft1DGpuTest, Accumulate) {
  ...
  const dca::linalg::util::CudaStream stream;
...
TEST_F(Dnfft1DGpuCmplxTest, AccumulateComplex) {
  prepareConfiguration(configuration_, M_, 128);

  dca::linalg::Matrix<std::complex<double>, dca::linalg::GPU> M_dev(M_);

  constexpr int oversampling = 8;
  // Compute f(w) using the delayed-NFFT algorithm on the CPU.
  dca::math::nfft::Dnfft1D<std::complex<double>, FreqDmn, LabelDmn, oversampling, dca::math::nfft::CUBIC>
      cpu_dnfft_obj;
  function<std::complex<double>, dmn_variadic<FreqDmn, LabelDmn>> f_w_dnfft_cpu("f_w_dnfft_cpu");
  computeWithCpuDnfft(M_, configuration_, cpu_dnfft_obj, f_w_dnfft_cpu);
  ...
  dca::math::nfft::Dnfft1DGpu<std::complex<double>, FreqDmn, RDmn, oversampling, dca::math::nfft::CUBIC>
      gpu_dnfft_obj(beta_, stream);
  function<std::complex<double>, dmn_variadic<FreqDmn, LabelDmn>> f_w_dnfft_gpu("f_w_dnfft_gpu");
