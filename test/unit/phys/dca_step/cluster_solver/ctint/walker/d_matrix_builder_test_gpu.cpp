...
using HostMatrixPair = std::array<HostMatrix, 2>;
...
TEST_F(G0Setup, RemoveAndInstertVertex) {
  ...
  dca::phys::solver::ctint::DeviceWorkspace workspace;
  ...
  for (int size : sizes) {
    ...
    builder.buildSQR(S, Q, R, workspace, configuration, 0, 0);
    builder_cpu.buildSQR(S2, Q2, R2, configuration);

    for (int s = 0; s < 2; ++s) {
      dca::linalg::util::syncStream(0, s);
