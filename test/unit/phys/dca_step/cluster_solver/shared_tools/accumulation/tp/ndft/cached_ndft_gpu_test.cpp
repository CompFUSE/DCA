...
double computeWithFastNDFT(const CachedNdftCpuTest::Configuration& config,
...
TEST_F(CachedNdftCpuTest, ExecuteGpu) {
...
double computeWithFastNDFT(const CachedNdftCpuTest::Configuration& config,
  ...
  dca::phys::solver::accumulator::CachedNdft<double, CachedNdftCpuTest::RDmn, CachedNdftCpuTest::FreqDmn,
                                             CachedNdftCpuTest::PosFreqDmn, dca::linalg::GPU>
      nft_obj(queue);
  ...
  // r2, w1, w2).
  const int nb = CachedNdftCpuTest::BDmn::dmn_size();
  const int nr = CachedNdftCpuTest::RDmn::dmn_size();
