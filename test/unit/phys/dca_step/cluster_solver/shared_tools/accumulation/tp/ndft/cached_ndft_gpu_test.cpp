...
TEST(CachedNdftTest, execute) {
  ...
  // Check errors.
  const auto err = dca::func::util::difference(*global::f_w_dft, f_w_fast);
...
double computeWithFastDNFT(
  ...
  dca::phys::solver::accumulator::CachedNdft<double, RDmn, FreqDmn, FreqDmnPos, dca::linalg::GPU> nft_obj(
      queue);
