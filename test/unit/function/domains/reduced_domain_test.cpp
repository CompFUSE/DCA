...
TEST(ReducedDomainTest, InitializeWithRange) {
  ...
  EXPECT_EQ(check2, ReducedDmn2::get_elements());
  EXPECT_EQ("dmn<6, long>_reduced", ReducedDmn2::get_name());

  // last = base-domain-size
  using BaseDmn3 = dmn<7, long>;
  ...
  EXPECT_EQ("dmn<7, long>_reduced", ReducedDmn3::get_name());

  // first = last
  using BaseDmn4 = dmn<8, long>;
  ...
  EXPECT_EQ("dmn<8, long>_reduced", ReducedDmn4::get_name());
}

TEST(ReducedDomainTest, InitializeWithRangeExceptions) {
  // first >= base-domain-size
  using BaseDmn1 = dmn<6, double>;
