...
//
// Description: test of pthread jacket with a dummy cluster solver.
...
void test_posix_initialization(int n_walk, int n_acc) {
  ...
  p_solver.integrate();
  std::cout << "With  nr walkers/accumulators: " << n_walk << "  " << n_acc << std::endl;
  ...
  for (int i = 0; i < n_walk; i++)
    ASSERT_EQ(MockWalker::available_walker_ids[i], false);
  long int info;
