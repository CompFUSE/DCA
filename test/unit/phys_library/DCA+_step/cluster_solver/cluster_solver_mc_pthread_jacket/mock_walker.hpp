...
public:
  ...
  std::vector<int> get_configuration() const { return fake_configuration; }
        ...
        : pars(pars0), rng(rng0), thread_id(id0), fake_configuration(1, 1)
{
  ...
  if(w_id>=nr_w or w_id<0) throw std::logic_error("out of bond index");
  pthread_mutex_lock(&walker_lock);
  if(not available_walker_ids[w_id]) throw std::logic_error("trying to use same id twice");
