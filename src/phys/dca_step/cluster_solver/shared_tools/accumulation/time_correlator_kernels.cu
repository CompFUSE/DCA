...
void computeG0(linalg::MatrixView<Scalar, linalg::GPU>& g0_mat,
               const DeviceInterpolationData<Scalar, SignType<Scalar>> g0, const Real* t_l,
...
// Instantation.
  template<> void computeG0<double, double>(linalg::MatrixView<double, linalg::GPU>&,
  ...
  template<> void computeG0<float, float>(linalg::MatrixView<float, linalg::GPU>&,
					  const DeviceInterpolationData<float, std::int8_t>, const float*,
                                      const int*, const int*, const float*, const int*, const int*,
                                      const GpuStream);
  template<> void computeG0<std::complex<double>, double>(
  ...
  template<> void computeG0<std::complex<float>, float>(
