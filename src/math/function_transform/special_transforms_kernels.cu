...
std::array<dim3, 2> getBlockSize(const int i, const int j) {
  ...
  const int n_blocks_i = dca::util::ceilDiv(i, n_threads_i);
  const int n_blocks_j = dca::util::ceilDiv(j, n_threads_j);

  return std::array<dim3, 2>{dim3(n_blocks_i, n_blocks_j), dim3(n_threads_i, n_threads_j)};
}

template <typename Scalar>
__global__ void phaseFactorsAndRearrangeKernel(const Scalar* in, const int ldi, Scalar* out,
                                               const int ldo, const int nb, const int nk,
                                               const int nw, const Scalar* phase_factors) {
...
void phaseFactorsAndRearrange(const std::complex<Real>* in, const int ldi, std::complex<Real>* out,
  ...
  phaseFactorsAndRearrangeKernel<<<blocks[0], blocks[1], 0, stream>>>(
      castCuda(in), ldi, castCuda(out), ldo, nb, nk, nw, castCuda(phase_factors));
}
