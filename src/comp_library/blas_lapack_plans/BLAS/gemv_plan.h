...
gemv_plan<scalartype>::gemv_plan(int m, int n)
    : TRANS('n'), M(m), N(n), LDA(m), INCX(1), INCY(1), alpha(1.), beta(0.) {}
...
void gemv_plan<float>::execute_plan() {
  BLAS::sgemv_(&TRANS, &M, &N, &alpha, matrix, &LDA, vector_source, &INCX, &beta, vector_target,
               &INCY);
}

template <>
void gemv_plan<double>::execute_plan() {
  BLAS::dgemv_(&TRANS, &M, &N, &alpha, matrix, &LDA, vector_source, &INCX, &beta, vector_target,
               &INCY);
}

template <>
void gemv_plan<std::complex<float>>::execute_plan() {
  BLAS::cgemv_(&TRANS, &M, &N, &alpha, matrix, &LDA, vector_source, &INCX, &beta, vector_target,
               &INCY);
}

template <>
void gemv_plan<std::complex<double>>::execute_plan() {
  BLAS::zgemv_(&TRANS, &M, &N, &alpha, matrix, &LDA, vector_source, &INCX, &beta, vector_target,
               &INCY);
}
