...
void blas_gemm_t(char& /*TRANSA*/, char& /*TRANSB*/, int& /*M*/, int& /*N*/, int& /*K*/,
                 scalartype& /*alpha*/, scalartype* /*A*/, int& /*LDA*/, scalartype* /*B*/,
                 int& /*LDB*/, scalartype& /*beta*/, scalartype* /*C*/, int& /*LDC*/) {
  std::cout << __PRETTY_FUNCTION__ << std::endl;
  throw std::logic_error(__FUNCTION__);
}

template <>
void blas_gemm_t<float>(char& TRANSA, char& TRANSB, int& M, int& N, int& K, float& alpha, float* A,
                        int& LDA, float* B, int& LDB, float& beta, float* C, int& LDC) {
  BLAS::sgemm_(&TRANSA, &TRANSB, &M, &N, &K, &alpha, A, &LDA, B, &LDB, &beta, C, &LDC);
}

template <>
void blas_gemm_t<double>(char& TRANSA, char& TRANSB, int& M, int& N, int& K, double& alpha, double* A,
                         int& LDA, double* B, int& LDB, double& beta, double* C, int& LDC) {
  BLAS::dgemm_(&TRANSA, &TRANSB, &M, &N, &K, &alpha, A, &LDA, B, &LDB, &beta, C, &LDC);
}

template <>
void blas_gemm_t<std::complex<float>>(char& TRANSA, char& TRANSB, int& M, int& N, int& K,
...
template <>
void blas_gemm_t<std::complex<double>>(char& TRANSA, char& TRANSB, int& M, int& N, int& K,
