...
#include "blas_lapack_plans/BLAS/BLAS_C_wrappers.h"
...
gbmv_plan<scalartype>::gbmv_plan(int n)
    : TRANS('N'), M(n), N(n), KL(0), KU(0), alpha(1.), LDA(1), INCX(1), beta(0.), INCY(1) {}

template <typename scalartype>
gbmv_plan<scalartype>::gbmv_plan(int m, int n)
    : TRANS('N'), M(m), N(n), KL(0), KU(0), alpha(1.), LDA(1), INCX(1), beta(0.), INCY(1) {}
