...
#ifndef BASIC_CUBLAS_FUNCTIONS_CU_H
#define BASIC_CUBLAS_FUNCTIONS_CU_H
...
namespace LIN_ALG {
  ...
  void cublasCheckReturnCodeInternal(cublasStatus_t return_code, std::string function_name, std::string file_name, int line){
    if (return_code != CUBLAS_STATUS_SUCCESS)
        cublas_error_msg(return_code, function_name, file_name, line);

#ifdef DEBUG_CUDA
    cuda_check_for_errors(function_name, file_name, line);
#endif 
  }
  ...
  void cublas_error_msg(cublasStatus_t error, std::string function_name, std::string file_name, int line)
  {
    ...
    ss << "\terror in function : " << function_name << "\tfile-name : " << file_name;
    ss << "\t at line : " << line << "\t error (" << error << "): " << cublas_error_str(error) << "\n\n";
  ...
  void create_cublas_handle(int thread_id)
  {
    if(CUBLAS_STATUS_SUCCESS != cublasCreate(&get_thread_handle(thread_id)))
      {
        std::cerr << "CUBLAS: Handle not initialized" << std::endl;
	exit(-1);
      }
  }
  ...
  void destroy_cublas_handle(int thread_id)
  {
    if(CUBLAS_STATUS_SUCCESS != cublasDestroy(get_thread_handle(thread_id)))
      {
        std::cerr << "CUBLAS: Handle not destroyed" << std::endl;
	exit(-1);
      }
  }
  ...
  int get_version(int thread_id)
  {
    ...
    if(CUBLAS_STATUS_SUCCESS != cublasGetVersion(get_thread_handle(thread_id), &version))
      {
        std::cerr << "CUBLAS: Version not available (CUBLAS not initialized)" << std::endl;
	exit(-1);
      }
