...
#ifndef COMP_LIBRARY_LINALG_SRC_MATRIX_H
#define COMP_LIBRARY_LINALG_SRC_MATRIX_H
...
matrix<scalartype, device_name>::matrix(std::string str)
    : name(str), thread_id(-1), stream_id(-1), current_size(0, 0), global_size(64, 64), data(NULL) {
  global_size = comply_to_block_size(global_size);
...
matrix<scalartype, device_name>::matrix(std::string str, std::pair<int, int> c_s)
    : name(str), thread_id(-1), stream_id(-1), current_size(c_s), global_size(c_s), data(NULL) {
  global_size = comply_to_block_size(global_size);
...
template <typename scalartype, device_type device_name>
matrix<scalartype, device_name>::matrix(std::pair<int, int> c_s, std::pair<int, int> g_s)
    : name("unnamed matrix"),
      thread_id(-1),
      stream_id(-1),
      current_size(c_s),
...
matrix<scalartype, device_name>::matrix(std::string str, std::pair<int, int> c_s,
    ...
    : name(str), thread_id(-1), stream_id(-1), current_size(c_s), global_size(g_s), data(NULL) {
  global_size = comply_to_block_size(global_size);
...
matrix<scalartype, device_name>::matrix(matrix<other_scalartype, other_device_name>& other_matrix)
  ...
  COPY_FROM<other_device_name, device_name>::execute(
      other_matrix.get_ptr(), other_matrix.get_current_size(), other_matrix.get_global_size(), data,
      current_size, global_size);
}
...
template <typename scalartype, device_type device_name>
inline typename matrix<scalartype, device_name>::matrix_scalartype*& matrix<scalartype,
                                                                            device_name>::get_ptr() {
  return data;
}
...
void matrix<scalartype, device_name>::resize_rows(std::pair<int, int> new_current_size) {
  ...
  {
    COPY_FROM<device_name, device_name>::execute(data, current_size, global_size, new_data,
                                                 current_size, new_global_size);
  }
...
void matrix<scalartype, device_name>::copy_from(matrix<scalartype, other_device_name>& other_matrix) {
  ...
  COPY_FROM<other_device_name, device_name>::execute(
      other_matrix.get_ptr(), other_matrix.get_current_size(), other_matrix.get_global_size(),
...
void matrix<scalartype, device_name>::copy_from(matrix<scalartype, other_device_name>& other_matrix,
  ...
  const static device_type device_t =
      LIN_ALG::CUBLAS_DEVICE_NAME<other_device_name, device_name>::device_t;
  ...
  switch (copy_t) {
    ...
    case ASYNCHRONOUS:
      ...
      COPY_FROM<other_device_name, device_name>::execute(
          other_matrix.get_ptr(), other_matrix.get_current_size(), other_matrix.get_global_size(),
          this->get_ptr(), this->get_current_size(), this->get_global_size(), thread_id, stream_id);
...
template <typename scalartype, device_type device_name>
template <device_type other_device_name>
scalartype matrix<scalartype, device_name>::difference(
    matrix<scalartype, other_device_name>& other_matrix) {
  ...
  COPY_FROM<device_name, CPU>::execute(this->get_ptr(), this->get_current_size(),
                                       this->get_global_size(), cp_this.get_ptr(),
                                       cp_this.get_current_size(), cp_this.get_global_size());

  COPY_FROM<other_device_name, CPU>::execute(
      other_matrix.get_ptr(), other_matrix.get_current_size(), other_matrix.get_global_size(),
...
template <device_type other_device_name>
scalartype matrix<scalartype, device_name>::difference(
    matrix<scalartype, other_device_name>& other_matrix, int N) {
  if (this->get_current_size() != other_matrix.get_current_size()) {
    throw std::logic_error("different matrix size");
  }
...
void matrix<scalartype, device_name>::print_fingerprint() {
  ...
  ss << "\t  memory-size  : " << global_size.first * global_size.second * sizeof(scalartype) * 1.e-6
     << "(Mbytes)\n";
  ss << "\n\n";
...
#endif  // COMP_LIBRARY_LINALG_SRC_MATRIX_H
