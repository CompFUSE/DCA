...
public:
  ...
  static void execute(char transa, char transb, scalartype alpha, matrix<scalartype, device_name>& a,
    ...
    if (transa == 'N') {
      assert(a.size().first == m);
      k = a.size().second;
    }
    else {
      assert(a.size().second == m);
      k = a.size().first;
    }

    if (transb == 'N') {
      assert(b.size().first == k);
      assert(b.size().second == n);
    }
    else {
      assert(b.size().second == k);
      assert(b.size().first == n);
    }
    ...
    dca::linalg::blas::UseDevice<device_name>::gemm(&transa, &transb, m, n, k, alpha, a.ptr(),
                                                    lda, b.ptr(), ldb, beta, c.ptr(), ldc,
                                                    thread_id, stream_id);
  }
  ...
  /*
                          ...
                          matrix<std::complex<scalartype>, device_name>& C)
      {
        ...
        matrix<scalartype, device_name> B_re("B_re", B.size(), B.capacity());
        matrix<scalartype, device_name> B_im("B_im", B.size(), B.capacity());
        ...
        matrix<scalartype, device_name> C_re("C_re", C.size(), C.capacity());
        matrix<scalartype, device_name> C_im("C_im", C.size(), C.capacity());
                          ...
                          matrix<std::complex<double>, device_name>& C)
      {
        ...
        matrix<double, device_name> A_re("A_re", A.size(), A.capacity());
        matrix<double, device_name> A_im("A_im", A.size(), A.capacity());
        ...
        matrix<double, device_name> C_re("C_re", C.size(), C.capacity());
        matrix<double, device_name> C_im("C_im", C.size(), C.capacity());
