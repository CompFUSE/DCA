...
vector<scalartype, device_name>::vector(std::string str)
    : name(str), thread_id(-1), stream_id(-1), current_size(0), global_size(64), data(NULL) {
  MEMORY_MANAGEMENT<device_name>::allocate(data, global_size);
  MEMORY_MANAGEMENT<device_name>::set_to_zero(data, global_size);
}
...
void vector<scalartype, device_name>::set(std::vector<scalartype>& other_vector,
  ...
  const static device_type device_t =
      LIN_ALG::CUBLAS_DEVICE_NAME<LIN_ALG::CPU, device_name>::device_t;
...
void vector<scalartype, device_name>::set(vector<scalartype, other_device_name>& other_vector,
  ...
  switch (copy_t) {
    ...
    case ASYNCHRONOUS:
      ...
      COPY_FROM<other_device_name, device_name>::execute(other_vector.get_ptr(), this->get_ptr(),
                                                         current_size, thread_id, stream_id);
...
template <typename scalartype, device_type device_name>
inline typename vector<scalartype, device_name>::vector_scalartype* vector<scalartype,
                                                                           device_name>::get_ptr() {
  return data;
}
...
template <typename scalartype, device_type device_name>
template <device_type other_device_name>
scalartype vector<scalartype, device_name>::difference(
    vector<scalartype, other_device_name>& other_vector) {
  ...
  COPY_FROM<other_device_name, CPU>::execute(other_vector.get_ptr(), cp_other.get_ptr(),
                                             current_size);
