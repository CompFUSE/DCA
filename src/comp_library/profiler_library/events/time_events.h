...
#ifndef PROFILER_TIME_EVENTS_H
#define PROFILER_TIME_EVENTS_H
...
namespace PROFILER 
{
  ...
  public:
    ...
    time_event(std::vector<scalartype>& my_counter,
	       int                      id);
  ...
  private:
    ...
    void update_counter(std::vector<scalartype>& counters,
  ...
  template<typename scalartype>
  time_event<scalartype>::time_event(std::vector<scalartype>& ourCountr,
				     int id):
    wallTime(),
    userTime(),
    systemTime(userTime.usage),
    ourCounter(ourCountr),
    thread_id(id)
  {}

  template<typename scalartype>
  time_event<scalartype>::~time_event()
  {}
  ...
  template<typename scalartype>
  void time_event<scalartype>::update_counter(std::vector<scalartype>& counters,
					      Duration wallDuration,
					      Duration userDuration,
					      Duration systemDuration) 
  {
