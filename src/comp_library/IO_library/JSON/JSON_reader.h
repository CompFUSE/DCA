...
{  
  ...
  template<typename scalar_type>
  void reader<IO::JSON>::execute(std::string name, LIN_ALG::vector<std::complex<scalar_type>, LIN_ALG::CPU>& V, 
				 const JsonAccessor& current_result, size_t index)
  {
    if(index==my_paths.size())
      {
  ...
  template<typename scalar_type>
  void reader<IO::JSON>::execute(std::string name, LIN_ALG::matrix<std::complex<scalar_type>, LIN_ALG::CPU>& A, 
				 const JsonAccessor& current_result, size_t index)
  {
    if(index==my_paths.size())
      {
