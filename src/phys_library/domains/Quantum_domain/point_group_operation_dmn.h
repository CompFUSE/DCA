  ...
  t(NULL)
{
  ...
  memset(O, 0, square(DIMENSION)*sizeof(double));
  ...
  t(NULL)
{
  ...
  memcpy(O, other.O, square(DIMENSION)*sizeof(double));
}
...
{
  static std::string name = "point-group-symmetry-domain (" + 
    to_str(symmetry_group_level) + ", " + base_cluster_type::get_name() + ")";
  
  return name;
}

template<symmetry_group_level_type symmetry_group_level, typename base_cluster_type>
std::vector<point_group_symmetry_element>& point_group_symmetry_domain<symmetry_group_level, base_cluster_type>::get_elements()
{
  static std::vector<point_group_symmetry_element> v(get_size(), point_group_symmetry_element(DIMENSION));
  return v;
}
...
template<symmetry_group_level_type symmetry_group_level, typename base_cluster_type>
std::vector<double> point_group_symmetry_domain<symmetry_group_level, base_cluster_type>::transform(int i, std::vector<double> vec)
{
  assert(i>-1);
  assert(i<get_size());
