...
template<typename scalar_type, int D, CLUSTER_NAMES N, CLUSTER_REPRESENTATION R, CLUSTER_SHAPE S>
std::vector<std::vector<scalar_type> >& centered_cluster_domain<cluster_domain<scalar_type, D, N, R, S> >::get_basis_vectors()
{
...
template<typename scalar_type, int D, CLUSTER_NAMES N, CLUSTER_REPRESENTATION R, CLUSTER_SHAPE S>
std::vector<std::vector<scalar_type> >& centered_cluster_domain<cluster_domain<scalar_type, D, N, R, S> >::get_super_basis_vectors()
{
...
{
  static std::string name = "centered-cluster-domain " +to_str(NAME)+" "+to_str(REPRESENTATION)+" "+to_str(SHAPE)+" (DIMENSION : " + to_str(DIMENSION) + ")";
  return name;
}
...
template<typename scalar_type, int D, CLUSTER_NAMES N, CLUSTER_REPRESENTATION R, CLUSTER_SHAPE S>
std::vector<std::vector<scalar_type> >& centered_cluster_domain<cluster_domain<scalar_type, D, N, R, S> >::get_elements()
{
...
{
  //cout << "\n\n\t start initialization of " << get_name() << "\t" << print_time();
  ...
  if(not is_initialized())
	...
	std::vector<std::vector<double> > vecs = cluster_operations::equivalent_vectors(vec, cluster_type::get_super_basis_vectors());
...
// 	get_elements().push_back(vecs[0]);
  ...
  //cout << "\n\n\t end initialization of " << get_name() << "\t" << print_time();
}
