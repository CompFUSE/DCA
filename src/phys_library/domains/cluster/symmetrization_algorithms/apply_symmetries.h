...
#ifndef APPLY_SYMMETRIES_H
#define APPLY_SYMMETRIES_H
...
struct apply_symmetries
{
  ...
  const static int INDEX = Length<point_group_type_list>::value;
  ...
  {
    apply_symmetry<cluster_type, point_group_type_list, INDEX-1>::reduce_cluster_coordinates(cluster_reduction);
  }
  ...
  static void reduce_cluster_pairs(cluster_reduction_type& cluster_reduction)
  {
	...
	if(i == cluster_reduction.get_irreducible_index(i)) // is an irreducible point
	  ...
	  {
	    apply_symmetry<cluster_type, point_group_type_list, INDEX-1>::reduce_cluster_pair(i, cluster_reduction);
	  }
      }
  }
};
