...
class Bett_cluster_square_2D_layered
{
public:
...
template<typename DCA_point_group_type>
double* Bett_cluster_square_2D_layered<DCA_point_group_type>::initialize_r_DCA_basis()
{
...
template<typename DCA_point_group_type>
double* Bett_cluster_square_2D_layered<DCA_point_group_type>::initialize_k_DCA_basis()
{
...
template<typename DCA_point_group_type>
double* Bett_cluster_square_2D_layered<DCA_point_group_type>::initialize_r_LDA_basis()
{
...
template<typename DCA_point_group_type>
double* Bett_cluster_square_2D_layered<DCA_point_group_type>::initialize_k_LDA_basis()
{
...
template<typename DCA_point_group_type>
template<class domain, class parameters_type>
void Bett_cluster_square_2D_layered<DCA_point_group_type>::initialize_H_interaction(FUNC_LIB::function<double , domain >& H_interaction,
										    parameters_type&            parameters)
{
...
template<typename DCA_point_group_type>
std::vector<std::pair<std::pair<int,int>, std::pair<int,int> > > Bett_cluster_square_2D_layered<DCA_point_group_type>::get_orbital_permutations()
{
...
template<typename DCA_point_group_type>
template<class domain>
void Bett_cluster_square_2D_layered<DCA_point_group_type>::initialize_H_symmetry(FUNC_LIB::function<int , domain>& H_symmetries)
{
  // e_up <==> e_dn symmetry
...
template<typename DCA_point_group_type>
template<class parameters_type>
std::complex<double> Bett_cluster_square_2D_layered<DCA_point_group_type>::get_LDA_Hamiltonians(parameters_type& parameters, 
												std::vector<double> k, 
												int b1, 
												int s1, 
												int b2, 
												int s2)
{
  ...
  for(size_t i=0; i<t_ij.size(); i++)
    if(t_ij[i][0]==b1 && t_ij[i][1]==s1 && t_ij[i][2]==b2 && t_ij[i][3]==s2 && t_ij[i][4]==0)
      t = t_ij[i][5];
