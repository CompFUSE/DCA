...
#ifndef SQUARE_A_2D_H_
#define SQUARE_A_2D_H_
...
double* square_A_2D::initialize_r_DCA_basis()
{
...
double* square_A_2D::initialize_k_DCA_basis()
{
...
double* square_A_2D::initialize_r_LDA_basis()
{
...
double* square_A_2D::initialize_k_LDA_basis()
{
...
template<class domain, class parameters_type>
void square_A_2D::initialize_H_interaction(FUNC_LIB::function<double , domain >& H_interaction,
					   parameters_type&            parameters)
{
...
std::complex<double> square_A_2D::get_LDA_Hamiltonians(double t, std::vector<double> k, int b1, int s1, int b2, int s2)
{
...
void square_A_2D::symmetrize_Hamiltonian(std::complex<double>* H_matrix)
{
...
/*double* square_A_2D::get_interaction_matrix()
{
  static double* H = new double[(2*BANDS)*(2*BANDS)];

  H[0+0*(2*BANDS)] = 0;    H[0+1*(2*BANDS)] = 1;
  H[1+0*(2*BANDS)] = 1;    H[1+1*(2*BANDS)] = 0;

  return H;
  }*/

std::vector<int> square_A_2D::get_interacting_bands()
{
