...
#ifndef GRAPHENE_MODEL_H
#define GRAPHENE_MODEL_H
...
double* graphene_model<DCA_point_group_type>::initialize_r_DCA_basis()
{
  ...
  r_DCA[0] = 1.5;  r_DCA[1] = 0.866025403784;
  r_DCA[2] = 1.5;  r_DCA[3] = -0.866025403784;
...
double* graphene_model<DCA_point_group_type>::initialize_k_DCA_basis()
{
  ...
  k_DCA[0] = 2.09439466667;  k_DCA[1] = 3.62759797377;
  k_DCA[2] = 2.09439466667;  k_DCA[3] = -3.62759797377;
...
double* graphene_model<DCA_point_group_type>::initialize_r_LDA_basis()
{
  ...
  r_LDA[0] = 1./2.; r_LDA[1] = 0;
  r_LDA[2] = 0.   ; r_LDA[3] = sqrt(3.)/2.;
...
double* graphene_model<DCA_point_group_type>::initialize_k_LDA_basis()
{
  ...
  k_LDA[2] = 0.    ; k_LDA[3] = 4*M_PI/sqrt(3.);
...
template<typename DCA_point_group_type>
std::vector<std::pair<std::pair<int,int>, std::pair<int,int> > > graphene_model<DCA_point_group_type>::get_orbital_permutations()
{
...
template<typename DCA_point_group_type>
template<class parameters_type>
std::complex<double> graphene_model<DCA_point_group_type>::get_LDA_Hamiltonians(parameters_type& parameters,
										std::vector<double> k, 
										int b1, int s1, int b2, int s2)
{
  ...
  if(s1==s2)
  {
	...
	if(b1==0 && b2==1)
	  H_LDA = t*(0.+std::exp(i*(k[0]*(-1.0)+k[1]*(0.0)))+std::exp(i*(k[0]*(0.5)+k[1]*(-0.866025403784)))+std::exp(i*(k[0]*(0.5)+k[1]*(0.866025403784))));

	if(b1==1 && b2==0)
	  H_LDA = t*(0.+std::exp(i*(k[0]*(-0.5)+k[1]*(-0.866025403784)))+std::exp(i*(k[0]*(-0.5)+k[1]*(0.866025403784)))+std::exp(i*(k[0]*(1.0)+k[1]*(0.0))));
