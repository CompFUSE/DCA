...
private:
    ...
    static std::vector<int> make_correlated_orbitals(parameters_type& parameters, H_interaction_type& H_interaction);
...
template <class parameters_type, class H_interaction_type>
std::vector<int> general_interaction::make_correlated_orbitals(
          ...
          for (int s_i = 0; s_i < s::dmn_size(); ++s_i) {
            for (int b_i = 0; b_i < b::dmn_size(); ++b_i) {
              if (std::abs(H_interaction(b_i, s_i, b_j, s_j, delta_r)) >
                  1.e-3) {
                int linear_index =
                        get_domain()(b_i, s_i, b_j, s_j, r_i, r_j);
