...
#ifndef PHYS_LIBRARY_PARAMETERS_PARAMETERS_SPECIALIZATION_SOLVER_SPECILIZATIONS_MC_SOLVER_CT_AUX_PARAMETERS_H
#define PHYS_LIBRARY_PARAMETERS_PARAMETERS_SPECIALIZATION_SOLVER_SPECILIZATIONS_MC_SOLVER_CT_AUX_PARAMETERS_H
...
template <class concurrency_type>
void MC_solver_parameters<DCA::CT_AUX_CLUSTER_SOLVER>::pack(concurrency_type& concurrency,
...
template <class concurrency_type>
void MC_solver_parameters<DCA::CT_AUX_CLUSTER_SOLVER>::unpack(concurrency_type& concurrency,
                                                              int* buffer, int buffer_size,
                                                              int& position) {
...
void MC_solver_parameters<DCA::CT_AUX_CLUSTER_SOLVER>::read_write(read_write_type& read_write_obj) {
  try {
    ...
    try {
      read_write_obj.execute("submatrix-size", submatrix_size);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("initial-matrix-size", initial_matrix_size);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("K-parameter", K_parameter);
    }
    catch (const std::exception& r_e) {
    }
...
#endif  // PHYS_LIBRARY_PARAMETERS_PARAMETERS_SPECIALIZATION_SOLVER_SPECILIZATIONS_MC_SOLVER_CT_AUX_PARAMETERS_H
