...
template <class concurrency_type>
void DCA_Parameters::unpack(concurrency_type& concurrency, int* buffer, int buffer_size,
                            int& position) {
...
void DCA_Parameters::read_write(read_write_type& read_write_obj) {
  try {
    ...
    try {
      read_write_obj.execute("do-DCA+", do_DCA_plus);
    }
    catch (const std::exception& r_e) {
    }

    try {
      read_write_obj.execute("interacting-bands", interacting_bands);
    }
    catch (const std::exception& r_e) {
    }

    try {
      read_write_obj.execute("DCA-iterations", DCA_iterations);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("DCA-accuracy", DCA_accuracy);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("DCA-mixing-factor", DCA_mixing_factor);
    }
    catch (const std::exception& r_e) {
    }

    try {
      read_write_obj.execute("cluster", DCA_cluster);
    }
    catch (const std::exception& r_e) {
    }

    {
      ...
      try {
        read_write_obj.execute("k-mesh-refinement", k_mesh_refinement);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("quadrature-rule", quadrature_rule);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("number-of-periods", number_of_periods);
      }
      catch (const std::exception& r_e) {
      }

      try {
        read_write_obj.execute("number-of-threads", nr_coarsegraining_threads);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("number-of-tail-frequencies", nr_tail_frequencies);
      }
      catch (const std::exception& r_e) {
      }

      try {
        read_write_obj.execute("precompute-Hamiltonian", precompute_Hamiltonian);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("phi(k) integration accuracy", phi_k_integration_accuracy);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("print-phi(k)", print_phi_k_to_python);
      }
      catch (const std::exception& r_e) {
      }
...

    {
      ...
      try {
        read_write_obj.execute("interpolation-method", interpolation_method);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("HTS-approximation", HTS_approximation);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("deconvolution-tolerance", deconvolution_tolerance);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("max-deconvolution-iterations", max_deconvolution_iterations);
      }
      catch (const std::exception& r_e) {
      }
