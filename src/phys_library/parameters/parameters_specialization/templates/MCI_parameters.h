...
template <class concurrency_type>
void MCI_parameters::unpack(concurrency_type& concurrency, int* buffer, int buffer_size,
                            int& position) {
...
void MCI_parameters::read_write(read_write_type& read_write_obj) {
  try {
    ...
    try {
      read_write_obj.execute("Sigma-file", Sigma_file);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("warm-up-sweeps", warm_up_sweeps);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("sweeps-per-measurement", number_of_sweeps_per_measurement);
    }
    catch (const std::exception& r_e) {
    }
    try {
      read_write_obj.execute("measurements", measurements);
    }
    catch (const std::exception& r_e) {
    }
...

    {
      ...
      try {
        read_write_obj.execute("nr-walkers", nr_walkers);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("nr-accumulators", nr_accumulators);
      }
      catch (const std::exception& r_e) {
      }
      try {
        read_write_obj.execute("additional-steps", additional_steps);
      }
      catch (const std::exception& r_e) {
      }

      try {
        read_write_obj.execute("HTS-threads", nr_HTS_threads);
      }
      catch (const std::exception& r_e) {
      }
...
#endif  // PHYS_LIBRARY_PARAMETERS_PARAMETERS_SPECIALIZATION_TEMPLATES_MCI_PARAMETERS_H
