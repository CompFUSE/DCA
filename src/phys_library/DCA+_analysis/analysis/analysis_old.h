...
#include "type_definitions.h"
  ...
  private:
    ...
    void interpolate_G4_b_k_w__b_k_w();
    
    void subtract_diagonal_G4_b_b_k_k_w_w();
    void add_diagonal_G4_b_b_k_k_w_w();

    void subtract_diagonal_G4_b_k_w__b_k_w();
    void add_diagonal_G4_b_k_w__b_k_w();
...
//     leading_V.to_JSON(ss);
//     ss << ",\n";

//     leading_P0.to_JSON(ss);
//     ss << ",\n";
	...
	for(int k1=0; k1<k_DCA::dmn_size(); k1++)
	  for(int l2=0; l2<b::dmn_size(); l2++)
	    for(int l1=0; l1<b::dmn_size(); l1++)
	      chi_0_function(l1,l2,k1,w1) = chi_0_b_k_w__b_k_w(l1,l2,k1,w1, l1,l2,k1,w1);
    }
...
//     diagrammatic_symmetries_obj.execute(G4_0_b_k_w__b_k_w);
    ...
    {
//       cout << "symmetrize G4_b_k_w__b_k_w" << endl;
//       symmetrize::execute(G4_b_k_w__b_k_w, MOMS.H_symmetry, parameters.get_q_vector(), true);
...
// 	  int lambda=eigenvals[eigenvals.size()-1-i].second;
// 	  std::complex<double> tmp=0;
// 	  for(int l1=0; l1<matrix_dim; ++l1)
// 	    for(int l0=0; l0<matrix_dim; ++l0)
// 	      tmp += full_chi_0_b_k_w__b_k_w(l0,l0)*eigensystem_pln.W[l0+lambda*matrix_dim]*conj(eigensystem_pln.W[l1+lambda*matrix_dim]);
