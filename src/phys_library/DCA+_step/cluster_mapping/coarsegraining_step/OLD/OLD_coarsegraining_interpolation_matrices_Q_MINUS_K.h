...
#ifndef DCA_COARSEGRAIN_INTERPOLATION_MATRICES_Q_MINUS_K_H
#define DCA_COARSEGRAIN_INTERPOLATION_MATRICES_Q_MINUS_K_H

namespace DCA
{
  template<typename k_dmn, typename K_dmn>
  class interpolation_matrices<k_dmn, dmn_0<coarsegraining_domain<K_dmn, Q_MINUS_K> > >
  {  

    typedef typename k_dmn::parameter_type::dual_type r_dmn;

    typedef dmn_0<coarsegraining_domain<K_dmn, Q_MINUS_K> > q_dmn;
  ...
  void interpolation_matrices<scalar_type, k_dmn, dmn_0<coarsegraining_domain<K_dmn, Q_MINUS_K> > >::initialize(concurrency_type& concurrency, int Q_ind)
  {
    if(is_initialized())
      return;
  ...
  template<typename scalar_type, typename k_dmn, typename K_dmn>
  template<typename scalar_type_1, typename scalar_type_2>
  void interpolation_matrices<scalar_type, k_dmn, dmn_0<coarsegraining_domain<K_dmn, Q_MINUS_K> > >::cast(scalar_type_1& x, scalar_type_2& y)
  {
    x = y;
  }
  ...
  template<typename scalar_type, typename k_dmn, typename K_dmn>
  template<typename scalar_type_1, typename scalar_type_2>
  void interpolation_matrices<scalar_type, k_dmn, dmn_0<coarsegraining_domain<K_dmn, Q_MINUS_K> > >::cast(scalar_type_1& x, std::complex<scalar_type_2>& y)
  {
