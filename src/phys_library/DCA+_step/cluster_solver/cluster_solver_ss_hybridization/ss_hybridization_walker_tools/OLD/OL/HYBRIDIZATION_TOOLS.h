...
#ifndef HYBRIDIZATION_TOOLS_H
#define HYBRIDIZATION_TOOLS_H
...
#include "type_definitions.h"
  ...
  public :
    ...
    static void compute_M_up(Hybridization_vertex& new_segment,
			     int k, 
			     vertex_vertex_matrix_type& M, 
			     orbital_configuration_t&   segments_old, 
			     G& F, 
    ...
    static double det_rat_down(int k, vertex_vertex_matrix_type& M, orbital_configuration_t& segments_old, double & det_rat_sign); 

    template<typename vertex_vertex_matrix_type> 
    static void compute_M_down(int k,vertex_vertex_matrix_type& M);
  ...
  double HYBRIDIZATION_TOOLS::interpolate_F(int* coor_flavor, double tau, Hybridization_function_t& F) 
  {
    ...
    if (tau<0) {
      tau += time_domain_type::beta;
      sign=-1;
    }
    
    return sign*polynomial_interpolation<t,2>::execute(tau, F, coor);
  }
					 ...
					 double&                    overlap) 
  {
      ...
      for (size_t i=0; i<segments_old.size(); i++) 
	{
      ...
      compute_Q_prime(Q, M, Q_prime);
       ...
       det_rat += BLAS::ddot_(&M.get_current_size(), &R[0], &inc, &Q_prime[0], &inc);
    }
  ...
  void HYBRIDIZATION_TOOLS::compute_M_up(Hybridization_vertex& new_segment,
					 int k, 
					 vertex_vertex_matrix_type& M, 
					 orbital_configuration_t&   segments_old, 
					 Hybridization_function_t&  F, 
					 ...
					 double S_prime_inv) 
  {
    ...
    if(k < M.get_current_size())
      M.insert_row_and_column(k);
    else{
      assert(k == M.get_current_size());
      M.resize(M.get_current_size()+1);
    }
    
    int i_new;
    // fill Q_prime and R_prime
    for(int i=0; i<M.get_current_size()-1; i++){
      i_new = (i<k ? i : i+1);
      M(i_new,k) = Q_prime[i]/S_prime_inv;
      M(k,i_new) = R_prime[i]/S_prime_inv;
    }
   ...
   */
  template <typename vertex_vertex_matrix_type, typename orbital_configuration_t> 
  double HYBRIDIZATION_TOOLS::det_rat_down(int k, vertex_vertex_matrix_type& M, orbital_configuration_t& segments_old, double & det_rat_sign) 
  {
   ...
   */
  template<typename vertex_vertex_matrix_type>
  void HYBRIDIZATION_TOOLS::compute_M_down(int k, vertex_vertex_matrix_type& M) 
  {
    static int inc = 1;
	...
	BLAS::dcopy_(&M.get_current_size(), &M(k,0), &incy, &R_prime[0], &inc);

	compute_M(Q_prime,R_prime,-1./Q_prime[k],M);
	M.remove_row_and_column(k);
      }
    else
      M.get_current_size() = 0;
  }
