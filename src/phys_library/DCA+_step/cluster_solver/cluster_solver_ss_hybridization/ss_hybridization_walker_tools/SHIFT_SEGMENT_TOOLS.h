...
#include "type_definitions.h"
	...
	bool shift_segment_end_vertex(int this_flavor,
				      double mu,
				      double& sign,
				      function_type_1& M,
				      function_type_2& F);
	...
	template<typename function_type_1, typename function_type_2>
	bool shift_segment_start_vertex(int this_flavor,
					double mu,
					double& sign,
					function_type_1& M,
					function_type_2& F);
    ...
    template<typename hybridization_routines_type>
    shift_segment_tools<hybridization_routines_type>::shift_segment_tools(hybridization_routines_type& hybridization_routines_ref):
    ...
    template<typename hybridization_routines_type>
    template<typename function_type_1, typename function_type_2>
    bool shift_segment_tools<hybridization_routines_type>::shift_segment_end_vertex(int this_flavor,
										    double mu,
										    double& sign,
										    function_type_1& M,
										    function_type_2& F)
	...
	for (int i=0; i<FLAVORS; i++)
	{
	    ...
	    double other_length = hybridization_routines.compute_overlap(segment_insert, configuration.get_vertices(i), configuration.get_full_line(i), BETA)
		- hybridization_routines.compute_overlap(segment_remove, configuration.get_vertices(i), configuration.get_full_line(i), BETA);
	...
	det_rat = hybridization_routines.det_rat_shift_end(this_flavor, segment_insert, n, M(this_flavor), vertices, F, R, Q, det_rat_sign, overlap);

	if (log(rng.get_random_number()) < log(det_rat)+(length-length_old)*mu-otherlength_u)
	{
	    if(det_rat_sign < 0){
		M(this_flavor).print();
	    }
    ...
    template<typename hybridization_routines_type>
    template<typename function_type_1, typename function_type_2>
    bool shift_segment_tools<hybridization_routines_type>::shift_segment_start_vertex(int this_flavor,
										      double mu,
										      double& sign,
										      function_type_1& M,
										      function_type_2& F)
	...
	det_rat = hybridization_routines.det_rat_shift_start(this_flavor, segment_insert, n, M(this_flavor), vertices, F, R, Q, det_rat_sign, overlap);
	...
	if (log(rng.get_random_number()) < log(det_rat)+(length-length_old)*mu-otherlength_u)
	{
	    ...
	    if(n==0 and (segment_insert.t_end()-segment_insert.t_start())*(segment_remove.t_end()-segment_remove.t_start())<0) 
	    {
	    ...
	    if(n==size-1 and(segment_insert.t_end()-segment_insert.t_start())*(segment_remove.t_end()-segment_remove.t_start())<0) 
	    {
