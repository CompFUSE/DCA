...
#ifndef SS_HYBRIDIZATION_SEGMENT_TOOLS_H
#define SS_HYBRIDIZATION_SEGMENT_TOOLS_H
...
namespace DCA
{
  ...
  public:
    ...
    bool insert_segment(int this_flavor,
                          double mu,
                          double& sign,
                          function_type_1& M,
                          function_type_2& F);
    ...
    template<typename function_type_1, typename function_type_2>
    bool remove_segment(int this_flavor,
			double mu,
			double& sign,
			function_type_1& M,
			function_type_2& F);
  ...
  template<typename hybridization_routines_type>
  segment_tools<hybridization_routines_type>::segment_tools(hybridization_routines_type& hybridization_routines_ref):
  ...
  template<typename hybridization_routines_type>
  template<typename function_type_1, typename function_type_2>
  bool segment_tools<hybridization_routines_type>::insert_segment(int this_flavor,
                                                                    double mu,
                                                                    double& sign,
                                                                    function_type_1& M,
                                                                    function_type_2& F)
      ...
      { // t does not lie on a segment -> it's possible to insert a new one starting from t
        ...
        if(length < t_up){
          ...
          for (int i=0; i<spin_orbitals; i++) {
            ...
            double other_length = hybridization_routines.compute_overlap(segment_insert, configuration.get_vertices(i), configuration.get_full_line(i), beta);//compute_overlap(segment_insert, other_segments[i], other_full_line[i], beta);
		...
		position++;
	      ...
	      hybridization_routines.compute_M_up(position, position, M(this_flavor), vertices, F, R, Q, det_rat*overlap);
	      sign *= det_rat_sign;
  ...
  template<typename hybridization_routines_type>
  template<typename function_type_1, typename function_type_2>
  bool segment_tools<hybridization_routines_type>::remove_segment(int this_flavor,
                                                                    double mu,
                                                                    double& sign,
                                                                    function_type_1& M,
                                                                  function_type_2& /*F*/)
  {
    ...
    for (int i=0; i<spin_orbitals; i++) 
      {
	if (i==this_flavor) continue;
	double other_length = hybridization_routines.compute_overlap(segment_remove, configuration.get_vertices(i), configuration.get_full_line(i), beta);//compute_overlap(segment_remove, other_segments[i], other_full_line[i], beta);
	otherlength_u += other_length*MOMS.H_interactions(i, this_flavor, 0);
      }
    
    double log_prob, det_rat, det_rat_sign;
    
    det_rat = hybridization_routines.det_rat_down(position, position, M(this_flavor), vertices, det_rat_sign);
