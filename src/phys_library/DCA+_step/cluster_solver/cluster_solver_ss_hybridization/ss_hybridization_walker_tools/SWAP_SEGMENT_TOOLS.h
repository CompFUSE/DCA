...
#include "type_definitions.h"
  ...
  template<typename hybridization_routines_type>
  swap_segment_tools<hybridization_routines_type>::swap_segment_tools(hybridization_routines_type& hybridization_routines_ref):
                                                                        ...
                                                                        function_type_2& F)
  {
    ...
    double length_this = hybridization_routines.compute_overlap(full_segment, vertices     , configuration.get_full_line(i), beta);
    double length_swap = hybridization_routines.compute_overlap(full_segment, swap_vertices, configuration.get_full_line(j), beta);
    ...
    for (int k=0; k<spin_orbitals; k++) {
      if (k!=i && k!=j) {
        ...
        if(configuration.get_full_line(i)){
          overlap += hybridization_routines.compute_overlap(full_segment, configuration.get_vertices(k), configuration.get_full_line(k), beta);
        }
        else
          for (typename orbital_configuration_type::iterator it=vertices.begin(); it!=vertices.end(); it++) {
            overlap += hybridization_routines.compute_overlap(*it, configuration.get_vertices(k), configuration.get_full_line(k), beta);
          }
        ...
        if(configuration.get_full_line(j)){
          ...
          for (typename orbital_configuration_type::iterator it=swap_vertices.begin(); it!=swap_vertices.end(); it++) {
            overlap += hybridization_routines.compute_overlap(*it, configuration.get_vertices(k), configuration.get_full_line(k), beta);
          }
...
//    return -1;
  ...
  template<typename hybridization_routines_type>
  template<typename function_type_1, typename function_type_2>
  double swap_segment_tools<hybridization_routines_type>::construct_inverse(function_type_1& M,
                                                                            double beta,
                                                                            function_type_2& F,
                                                                            int flavor_1,
                                                                            int flavor_2)
  ...
  template<typename hybridization_routines_type>
  template<typename matrix_type_1, typename function_type_2>
  void swap_segment_tools<hybridization_routines_type>::construct_matrix(matrix_type_1& M,
                                                                         double beta,
                                                                         function_type_2& F,
                                                                         int flavor_1,
                                                                         int flavor_2)
  {
    ...
    for (typename orbital_configuration_type::iterator it1=segments.begin(); it1!=segments.end(); it1++) {
      row++;
      for (typename orbital_configuration_type::iterator it2=segments.begin(); it2!=segments.end(); it2++) {
        column++;
