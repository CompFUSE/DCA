...
namespace QMC {
   ...
   * \f}
   * where \f$P_l(x)\f$ are the legendre polynomials, and \f$G_l\f$ denote the coefficients of \f$G(\tau)\f$ in the Legendre basis and can be measured according to
   * \f{eqnarray*}{
   * G_{ab;l} = -\frac{\sqrt{2l+1}}{\beta} \left< \sum_{\alpha \beta = 1}^{n} M_{\alpha \beta} P_l(\tau'_{\alpha} - \tau_{\beta}) \delta_{a,\alpha} \delta_{b,\beta} \right>_{MC}
   * \f}
   * and equivalent for \f$F_{ab}^j(\tau)\f$
   * \f{eqnarray*}{
   * F_{ab;l}^j = -\frac{\sqrt{2l+1}}{\beta} \left< \sum_{\alpha \beta = 1}^{n} M_{\alpha \beta} P_l(\tau'_{\alpha} - \tau_{\beta})n_j(\tau_{\beta}) \delta_{a,\alpha} \delta_{b,\beta} \right>_{MC}
   * \f}
   * In the segment picture, the matrix element \f$n_j(\tau_{\beta})\f$ (one or zero) of this operator is simply determined by examining whether or not a segment is present in the timeline for flavor j at time \f$\tau_{\beta}\f$. Hence this quantity can be measured 
   * at essentially no additional computational cost. Note that this function only contributes if j is different from the index b (and \f$\beta\f$) and therefore \f$n_j\f$ is never evaluated at the position of the creator of flavor b at time  \f$\tau_{\beta}\f$.
   *
  ...
  template<class parameters_type, class base_cluster_type>
  MC_single_particle_accumulator<SS_CT_HYB, LEGENDRE, parameters_type, base_cluster_type>::MC_single_particle_accumulator(parameters_type&          parameters_ref):
  ...
  template<class parameters_type, class base_cluster_type>
  MC_single_particle_accumulator<SS_CT_HYB, LEGENDRE, parameters_type, base_cluster_type>::~MC_single_particle_accumulator()
  {}
  ...
  template<class parameters_type, class base_cluster_type>
  template<class stream_type>
  void MC_single_particle_accumulator<SS_CT_HYB, LEGENDRE, parameters_type, base_cluster_type>::to_JSON(stream_type& ss)
  {
    ss << ",";
    G_r_l.to_JSON(ss);
    ss << ",";
														     ...
														     int                  flavor)
  {
      ...
      else
	{
	  ...
	  for(int y = 0; y < int(vertices_j.size()); y++){
	    if(vertices_j[y].t_start() < vertices_j[y].t_end() && vertices_j[y].t_start() < t_start && t_start < vertices_j[y].t_end()){
	      U_times_n += 1./2.*(H_interactions(j,flavor,0)+H_interactions(flavor,j,0));
	    }
	    else if(vertices_j[y].t_end() < vertices_j[y].t_start() && (vertices_j[y].t_start() < t_start || t_start < vertices_j[y].t_end())){
	      U_times_n += 1./2.*(H_interactions(j,flavor,0)+H_interactions(flavor,j,0));
	    }
	  }
	}
