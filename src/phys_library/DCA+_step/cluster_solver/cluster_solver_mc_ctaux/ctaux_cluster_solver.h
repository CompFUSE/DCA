...
namespace DCA
{
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  template<IO::FORMAT DATA_FORMAT>
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::write(IO::writer<DATA_FORMAT>& writer)
  {
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  template<typename dca_info_struct_t>
  double cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::finalize(dca_info_struct_t& dca_info_struct)
  {
    {// Compute new Sigma
    ...
    dca_info_struct.thermalization_per_mpi_task(DCA_iteration) = thermalization_time/double(concurrency.number_of_processors());
    dca_info_struct.MC_integration_per_mpi_task(DCA_iteration) = MC_integration_time/double(concurrency.number_of_processors());

    dca_info_struct.times_per_mpi_task(DCA_iteration) = total_time             /double(concurrency.number_of_processors());
    dca_info_struct.Gflop_per_mpi_task(DCA_iteration) = accumulator.get_Gflop()/double(concurrency.number_of_processors());

    dca_info_struct.Gflops_per_mpi_task(DCA_iteration)
      = dca_info_struct.Gflop_per_mpi_task(DCA_iteration)/dca_info_struct.times_per_mpi_task(DCA_iteration);
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::warm_up(walker_type& walker)
  {
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::measure(walker_type& walker)
  {
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::update_shell(int i, int N, int N_k)
  {
    int tmp = i;
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::update_shell(int i, int N, int N_k, int N_s)
  {
    int tmp = i;
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::compute_error_bars(int Nb_measurements_per_node)
  {
  ...
  void cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::compute_G_k_w_from_M_r_w()
  {
    ...
    for(int k_ind=0; k_ind<k_DCA::dmn_size(); k_ind++){
      for(int w_ind=0; w_ind<w::dmn_size(); w_ind++){
        ...
        memcpy(G0_cluster_excluded_matrix, &MOMS.G0_k_w_cluster_excluded(0,0,0,0,k_ind,w_ind), sizeof(std::complex<double>)*matrix_size);
        memcpy(M_matrix                  , &accumulator.get_M_k_w()     (0,0,0,0,k_ind,w_ind), sizeof(std::complex<double>)*matrix_size);
  ...
  double cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::compute_S_k_w_from_G_k_w()
  {
    ...
    for(int k_ind=0; k_ind<k_DCA::dmn_size(); k_ind++){
      for(int w_ind=0; w_ind<w::dmn_size(); w_ind++){
        ...
        memcpy(Sigma_matrix, &MOMS.Sigma(0,0,0,0,k_ind,w_ind), sizeof(std::complex<double>)*matrix_size);

        {
          invert_plan<std::complex<double> > invert_pln(matrix_dim);
          memcpy(invert_pln.Matrix, &MOMS.G_k_w(0,0,0,0,k_ind,w_ind), sizeof(std::complex<double>)*matrix_size);
        ...
        {
          invert_plan<std::complex<double> > invert_pln(matrix_dim);
          memcpy(invert_pln.Matrix, &MOMS.G0_k_w_cluster_excluded(0,0,0,0,k_ind,w_ind), sizeof(std::complex<double>)*matrix_size);
          invert_pln.execute_plan();
        ...
        memcpy(&MOMS.Sigma(0,0,0,0,k_ind,w_ind), Sigma_matrix, sizeof(std::complex<double>)*matrix_size);
      }
    }
...
//WARNING this function is not implemented
  ...
  template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
  double cluster_solver<CT_AUX_CLUSTER_SOLVER, device_t, parameters_type, MOMS_type>::mix_self_energy(double alpha)
  {
    symmetrize::execute(MOMS.Sigma        , MOMS.H_symmetry);
    ...
    for(int w_ind=w::dmn_size()/2; w_ind<w::dmn_size()/2+offset; w_ind++){
      for(int k_ind=0; k_ind<k_DCA::dmn_size(); k_ind++){
        for(int l1=0; l1<b::dmn_size()*s::dmn_size(); l1++){
          ...
          diff_L2_norm += square(abs(MOMS.Sigma(l1,l1,k_ind,w_ind)
