...
#include "type_definitions.h"
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::MC_accumulator(parameters_type& parameters_ref,
                                                                                        MOMS_type&       MOMS_ref,
                                                                                        int              id):
      MC_accumulator_data(),
    ...
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::finalize()
    {
...

      {
        ...
        double factor = 1./std::sqrt(1+concurrency.number_of_processors()
                                     *parameters.get_nr_accumulators()
      ...
      if(parameters.do_equal_time_measurements())
	{
	  ...
	  magnetic_cluster_moment = MC_two_particle_equal_time_accumulator_obj.get_magnetic_cluster_moment();
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    std::vector<vertex_singleton>& MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::get_configuration(e_spin_states_type e_spin)
    {
      if(e_spin==e_UP)
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    template<IO::FORMAT DATA_FORMAT>
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::write(IO::writer<DATA_FORMAT>& writer)
    {
...
//       writer.close_group();
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    template<typename walker_type>
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::update_from(walker_type& walker)
    {
      ...
      if(QMC_INTEGRATOR_BIT)
	{
      ...
      copy(full_configuration.get(e_DN).begin(), full_configuration.get(e_DN).end(), HS_configuration_e_DN.begin());

      HS_configuration_e_UP.resize(full_configuration.get(e_UP).size());
      copy(full_configuration.get(e_UP).begin(), full_configuration.get(e_UP).end(), HS_configuration_e_UP.begin());
      ...
      //LIN_ALG::CUBLAS_THREAD_MANAGER<walker_type::walker_device_type>::synchronize_streams(walker.get_thread_id(), 0);
    }
     ...
     */
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::store_standard_deviation(int nr_measurements,
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::accumulate_single_particle_quantities()
    {
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::accumulate_equal_time_quantities()
    {
    ...
    template<LIN_ALG::device_type device_t, class parameters_type, class MOMS_type>
    void MC_accumulator<CT_AUX_SOLVER, device_t, parameters_type, MOMS_type>::accumulate_two_particle_quantities()
    {
      {
        profiler_type profiler("tp-accumulation nonlocal G", "CT-AUX accumulator", __LINE__, thread_id);

        accumulator_nonlocal_G_obj.execute(HS_configuration_e_UP, M_e_UP,
                                           HS_configuration_e_DN, M_e_DN);
      }
