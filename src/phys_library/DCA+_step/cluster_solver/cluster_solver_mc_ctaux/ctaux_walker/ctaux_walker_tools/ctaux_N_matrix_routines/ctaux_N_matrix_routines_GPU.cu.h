...
#ifndef DCA_QMCI_N_MATRIX_ROUTINES_GPU_CU_H
#define DCA_QMCI_N_MATRIX_ROUTINES_GPU_CU_H
...
namespace DCA
{
  namespace QMCI
  {
    namespace N_MATRIX_TOOLS_GPU_KERNELS 
    {
      ...
      __global__ void compute_G_cols_kernel(int N_i, int N_r, int N_c,
                                            int*    p_ptr,
                                            double* exp_V_ptr,
        ...
        if(N_r>0 and N_i>0)
          {
            ...
            compute_G_cols_kernel<<<blocks, threads, 0, stream_handle>>>(N_i,  N_r, N_c,
                                                                         p_ptr,       exp_V_ptr,
                                                                         N_ptr,       N_ld,
                                                                         G_ptr,       G_ld,
                                                                         G_cols_ptr,  G_cols_ld);
        ...
        if(N_i>0)
          {
            ...
            compute_d_vector_kernel<<<blocks, threads, 0, stream_handle>>>(N_i, d_ind, d_ptr, p_ptr, N_ptr, N_ld);
