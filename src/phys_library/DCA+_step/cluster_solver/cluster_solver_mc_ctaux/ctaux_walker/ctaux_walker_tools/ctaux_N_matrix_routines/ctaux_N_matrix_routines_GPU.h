...
public:
  ...
  void set_d_vector(std::vector<int>& d_index, LIN_ALG::matrix<double, LIN_ALG::GPU>& N,
  ...
  void compute_G_cols(std::vector<double>& exp_V, LIN_ALG::matrix<double, LIN_ALG::GPU>& N,
...
template <typename parameters_type>
N_MATRIX_TOOLS<LIN_ALG::GPU, parameters_type>::N_MATRIX_TOOLS(int id, parameters_type& parameters_ref)
    : thread_id(id),
      stream_id(0),
      ...
      identity("identity    N_MATRIX_TOOLS<LIN_ALG::GPU>",
               MAX_VERTEX_SINGLETS * parameters.get_K_PHANI()),
      permutation("permutation N_MATRIX_TOOLS<LIN_ALG::GPU>",
                  MAX_VERTEX_SINGLETS * parameters.get_K_PHANI()),
...
template <typename parameters_type>
void N_MATRIX_TOOLS<LIN_ALG::GPU, parameters_type>::set_d_vector(
    LIN_ALG::vector<double, LIN_ALG::CPU>& d_inv) {
...
void N_MATRIX_TOOLS<LIN_ALG::GPU, parameters_type>::scale_rows(LIN_ALG::matrix<double, LIN_ALG::GPU>& N) {
  ...
  LIN_ALG::SCALE<LIN_ALG::GPU>::many_rows(N_c, N_i, permutation.get_ptr(), d_vec.get_ptr(),
                                          N.get_ptr(), N_LD, thread_id, stream_id);
}

template <typename parameters_type>
double* N_MATRIX_TOOLS<LIN_ALG::GPU, parameters_type>::get_device_ptr(
...
template <typename parameters_type>
void N_MATRIX_TOOLS<LIN_ALG::GPU, parameters_type>::copy_rows(
    LIN_ALG::matrix<double, LIN_ALG::GPU>& N, LIN_ALG::matrix<double, LIN_ALG::GPU>& N_new_spins) {
  ...
  LIN_ALG::COPY<LIN_ALG::GPU>::many_rows(
...
template <typename parameters_type>
void N_MATRIX_TOOLS<LIN_ALG::GPU, parameters_type>::compute_G_cols(
    std::vector<double>& exp_V_CPU, LIN_ALG::matrix<double, LIN_ALG::GPU>& N,
    LIN_ALG::matrix<double, LIN_ALG::GPU>& G, LIN_ALG::matrix<double, LIN_ALG::GPU>& G_cols) {
  ...
  N_MATRIX_TOOLS_GPU_KERNELS::compute_G_cols(N_i, N_r, N_c, permutation.get_ptr(), exp_V.get_ptr(),
                                             N.get_ptr(), N.get_leading_dimension(), G.get_ptr(),
                                             G.get_leading_dimension(), G_cols.get_ptr(),
                                             G_cols.get_leading_dimension(), thread_id, stream_id);
}
