...
#ifndef DCA_QMCI_G_MATRIX_ROUTINES_GPU_CU_H
#define DCA_QMCI_G_MATRIX_ROUTINES_GPU_CU_H
...
namespace DCA
{
  namespace QMCI
  {
    namespace GPU_KERNELS_G_TOOLS
    {
      ...
      __global__ void read_G_matrix_kernel(int S, int vertex_index,
                                           int* i_index, int* j_index, bool* is_Bennett, double* exp_Vj,
                                           double* N_ptr, int LD_N,
                                           double* G_ptr, int LD_G,
      ...
      void read_G_matrix_elements(int S, int vertex_index,
                                  int*    i_ptr,
                                  int*    j_ptr,
                                  bool*   is_Bennett_ptr,
                                  double* exp_Vj_ptr,
                                  ...
                                  double* result_ptr, int incr)
      {
        ...
        read_G_matrix_kernel<<<1,S>>>(S, vertex_index, i_ptr, j_ptr, is_Bennett_ptr, exp_Vj_ptr, N_ptr, N_LD, G_ptr, G_LD, result_ptr, incr);
      ...
      __global__ void compute_row_on_Gamma_matrix_kernel(int     row_index,
                                                         int     vertex_index,
                                                         int*    indices,
                                                         double* exp_V,
                                                         double* N_ptr  , int LD_N,
      ...
      {
        //assert(S<get_number_of_threads());
        compute_row_on_Gamma_matrix_kernel<<<S,1>>>(row_index, vertex_index, indices, exp_V, N_ptr, LD_N, G_ptr, LD_G, row_ptr, incr);
      ...
      __global__ void compute_col_on_Gamma_matrix_kernel(int     col_index,
                                                         int     vertex_index,
                                                         int*    indices,
                                                         double* exp_V,
                                                         double* N_ptr  , int LD_N,
      ...
      {
        //assert(S<get_number_of_threads());
        compute_col_on_Gamma_matrix_kernel<<<S,1>>>(col_index, vertex_index, indices, exp_V, N_ptr, LD_N, G_ptr, LD_G, col_ptr, incr);
