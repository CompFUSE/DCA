...
#ifndef ADVANCED_FERMIONIC_FOCK_SPACE_H
#define ADVANCED_FERMIONIC_FOCK_SPACE_H
...
namespace DCA
{
  namespace ADVANCED_EXACT_DIAGONALIZATION
  {
    ...
    private:
      ...
      void factorize(symmetry_operation& Op, element_type& subspace, std::vector< element_type >& new_Hilbert_spaces,
    ...
    template<typename parameter_type, typename ed_options>
    Fock_space<parameter_type, ed_options>::Fock_space(bool occupation_symmetry, bool magnetization_symmetry)
    {
    ...
    template<typename parameter_type, typename ed_options>
    std::vector< Hilbert_space<parameter_type, ed_options> >& Fock_space<parameter_type, ed_options>::get_elements()
    {
    ...
    void Fock_space<parameter_type, ed_options>::apply_magnetization_symmetry()
    {
      ...
      for (int_type N_occ = 0; N_occ <= N_occ_max; ++N_occ){
        ...
        for (int_type j = 0; j < number_of_subspaces; ++j){
          ...
          for (typename std::vector< psi_state<parameter_type, ed_options> >::iterator
                 it = HS.psi_states_begin(); it != HS.psi_states_end(); ++it){
    ...
    void Fock_space<parameter_type, ed_options>::apply_translation_symmetry(std::string ED_method)
    {
      ...
      for (int k = 0; k < DIMENSION; ++k){
        ...
        std::vector<double> basis_vec = cluster_operations::translate_inside_cluster(basis[k], super_basis);
        ...
        for (int r = 0; r < r_dmn::dmn_size(); ++r){
          for (int s = 0; s < s_dmn::dmn_size(); ++s){
            for (int b = 0; b < b_dmn::dmn_size(); ++b){
              permutation_vector[idx] = indices[r]*s_dmn::dmn_size()*b_dmn::dmn_size() + s*b_dmn::dmn_size() + b;
              ++idx;
            }
          }
        }
    ...
    void Fock_space<parameter_type, ed_options>::apply_rotation_symmetry(std::string /*symmetries*/, std::string ED_method)
    {
      ...
      for (int l = 1; l<2; ++l){
        ...
        for (int r = 0; r < r_dmn::dmn_size(); ++r){
          for (int s = 0; s < s_dmn::dmn_size(); ++s){
            for (int b = 0; b < b_dmn::dmn_size(); ++b){
              permutation_vector[index] = r_symmetry_matrix(r,b, l).first*s_dmn::dmn_size()*b_dmn::dmn_size()
    ...
    template<typename parameter_type, typename ed_options>
    template<class symmetry_operation>
    void Fock_space<parameter_type, ed_options>::factorize(symmetry_operation&          Op,
                                                           element_type&                subspace,
                                                           std::vector< element_type >& new_Hilbert_spaces,
                                                           bool                         create_subspaces,
                                                           int                          k)
    {
