...
#ifndef ADVANCED_FERMIONIC_PSI_STATE_H
#define ADVANCED_FERMIONIC_PSI_STATE_H
...
namespace DCA
{
  namespace ADVANCED_EXACT_DIAGONALIZATION
  {
    ...
    public:
      ...
      std::vector< phi_state<parameter_type, ed_options, PHI_SINGLET> >  get_phi_obj() const { return phi_obj; }

      phi_state<parameter_type, ed_options, PHI_SINGLET>& get_phi_obj(int i)       { assert(i >= 0 && i < size()); return phi_obj[i]; }
      phi_state<parameter_type, ed_options, PHI_SINGLET>  get_phi_obj(int i) const { assert(i >= 0 && i < size()); return phi_obj[i]; }
    ...
    template<typename parameter_type, typename ed_options>
    psi_state<parameter_type, ed_options>::psi_state(const phi_state<parameter_type, ed_options, PHI_SINGLET>& phi_obj_):
      phi_obj(1, phi_obj_)
    {}
