...
#include "type_definitions.h"
    ...
    template<typename parameter_type, typename ed_options>
    fermionic_Hamiltonian<parameter_type, ed_options>::~fermionic_Hamiltonian()
    {}
        ...
        for(int r_j=0; r_j<r_dmn::dmn_size(); r_j++){
          for(int s_j=0; s_j<s_dmn::dmn_size(); s_j++){
            for(int b_j=0; b_j<b_dmn::dmn_size(); b_j++){
              ...
              V_obj.index = b_j+b_dmn::dmn_size()*(s_j+s_dmn::dmn_size()*r_j);      // index: r -> spin -> band
    ...
    template<typename parameter_type, typename ed_options>
    void fermionic_Hamiltonian<parameter_type, ed_options>::add_V_to_Hamiltonian_old(int N, matrix_type& H, Hilbert_space_type& subspace)
    {
            ...
            for (int k_i = 0; k_i < Psi_i.size(); ++k_i){
              ...
              if ( Psi_f.phis[k_f] == Psi_i.phis[k_i] ){
                for (size_t l = 0; l < V_i.size(); ++l){
                  H(state_f, state_i) += V_i[l].value * conj(Psi_f.coefficients[k_f]) * Psi_i.coefficients[k_i]
                    * (1.*Psi_i.phis[k_i][V_i[l].index]);
                }
              }
            }
          }
    ...
    template<typename parameter_type, typename ed_options>
    void fermionic_Hamiltonian<parameter_type, ed_options>::add_T_to_Hamiltonian_old(int N, matrix_type& H, Hilbert_space_type& subspace)
    {
      for (int state_f = 0; state_f<N; ++state_f){
        psi_state_type& Psi_f = subspace.get_element(state_f);
        for (int state_i = 0; state_i<N; ++state_i){
              ...
              if ( possible_hopping(Psi_i.phis[k_i], Psi_f.phis[k_f]) ){
                for (size_t l=0; l<t_ij.size(); l++){
                  if (Psi_i.phis[k_i][t_ij[l].rhs] == 1 and Psi_i.phis[k_i][t_ij[l].lhs] == 0 and
                    ...
                    H(state_f, state_i) += phase * conj(Psi_f.coefficients[k_f]) * Psi_i.coefficients[k_i] * t_ij[l].value;
                  }
                }
              }
            }
        ...
        for (int j = 0; j < rep.size(); ++j){
          ...
          if ( fermionic_operators_type::annihilate_at(t_ij[l].rhs, phi, sign) &&
            ...
            if (i < rep.size()){
              ...
              for (int c = 0; c < column_index.size(); ++c){
                for (int r = 0; r < row_index.size(); ++r){
                  H(row_index[r], column_index[c]) += conj(row_alpha[r]) * column_alpha[c] * scalar_type(sign) * t_ij[l].value;
                }
              }
            }
          }
    ...
    template<typename parameter_type, typename ed_options>
    void fermionic_Hamiltonian<parameter_type, ed_options>::add_U_to_Hamiltonian_old(int N, matrix_type& H, Hilbert_space_type& subspace)
    {
      for (int state=0; state<N; ++state){
        ...
        for (int k=0; k<Psi.size(); ++k){
          ...
          for (int l=0; l<U_ij.size(); ++l){
            H(state, state) += U_ij[l].value/scalar_type(4.) * conj(Psi.coefficients[k]) * Psi.coefficients[k];

            if(abs(Psi.phis[k][U_ij[l].lhs]-scalar_type(1.))<1.e-3)
              H(state, state) -= U_ij[l].value/scalar_type(2.) * conj(Psi.coefficients[k]) * Psi.coefficients[k];
    ...
    template<typename parameter_type, typename ed_options>
    void fermionic_Hamiltonian<parameter_type, ed_options>::add_U_to_Hamiltonian(int N, matrix_type& H, Hilbert_space_type& subspace)
    {
      Hilbert_space_phi_representation_type& rep = subspace.get_rep();
        ...
        for (int j = 0; j < rep.size(); ++j){
          ...
          if( fermionic_operators_type::annihilate_at(U_ij[l].rhs, phi, sign) &&
            ...
            if (i < rep.size()){
              ...
              for (int c = 0; c < column_index.size(); ++c){
                for (int r = 0; r < row_index.size(); ++r){
                  H(row_index[r], column_index[c]) += conj(row_alpha[r]) * column_alpha[c] * U_ij[l].value;
                }
              }
            }
          }
        ...
        for (int j = 0; j < rep.size(); ++j){
          ...
          if( fermionic_operators_type::annihilate_at(U_ij[l].rhs, phi, sign) &&
            ...
            if (i < rep.size()){
              ...
              for (int c = 0; c < column_index.size(); ++c){
                for (int r = 0; r < row_index.size(); ++r){
                  H(row_index[r], column_index[c]) -= conj(row_alpha[r]) * column_alpha[c] * U_ij[l].value/scalar_type(2.);
                }
              }
            }
          }
    ...
    template<typename parameter_type, typename ed_options>
    bool fermionic_Hamiltonian<parameter_type, ed_options>::check_block_structure(int N, matrix_type& H, Hilbert_space_type& subspace)
    {
      for (int state_f = 0; state_f < N; ++state_f){
        psi_state_type& Psi_f = subspace.get_element(state_f);
        for (int state_i = 0; state_i<N; ++state_i){
    ...
    void fermionic_Hamiltonian<parameter_type, ed_options>::diagonalize_Hamiltonians_st()
    {
      ...
      for (int i = 0; i < fermionic_Fock_dmn_type::dmn_size(); ++i){
...

        {
          ...
          int start = clock();
          LIN_ALG::GEEV<LIN_ALG::CPU>::execute('V', 'U', Hamiltonians(i), eigen_energies(i), eigen_states(i));
          int end = clock();
