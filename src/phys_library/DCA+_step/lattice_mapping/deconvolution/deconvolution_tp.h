...
namespace DCA
{
										  ...
										  FUNC_LIB::function<std::complex<scalartype>, dmn_2<dmn_4<b,b,target_k_dmn_t,w_VERTEX>, dmn_4<b,b,target_k_dmn_t,w_VERTEX> > >& Gamma_lattice_deconv)
  {
    ...
    LIN_ALG::matrix<std::complex<scalartype>, LIN_ALG::CPU> phi_inv("phi_inv", std::pair<int,int>(N, N));
    
    this->compute_T_inv_matrix(parameters.get_singular_value_cut_off(), phi_inv);
    
    math_algorithms::functional_transforms::TRANSFORM<k_dmn_t, target_k_dmn_t>::execute_on_all(Gamma_lattice_interp, Gamma_lattice_deconv, phi_inv);
  }
