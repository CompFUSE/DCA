...
#ifndef MATH_ALGORITHMS_NFFT_WINDOW_FUNCTION_KAISER_H
#define MATH_ALGORITHMS_NFFT_WINDOW_FUNCTION_KAISER_H
...
namespace MATH_ALGORITHMS
{
  namespace NFFT
  {
     ...
     *
     *  \author Peter Staar.
     *  \brief  This class implements the normalized Kaiser-Bessel window function.
     *
     ...
     *   \left\{
     *   \begin{array}{rl}
     *   \frac{1}{\pi\:\phi_0} \frac{\sinh(b\:\sqrt{m^2-(n\:\tau)^2})}{\sqrt{m^2-(n\:\tau)^2})} & \mbox{if } |\tau| < \frac{m}{n},\\
     *   \frac{1}{\pi\:\phi_0} \frac{\sin (b\:\sqrt{m^2-(n\:\tau)^2})}{\sqrt{m^2-(n\:\tau)^2})} & \mbox{if } |\tau| > \frac{m}{n}.
     *   \end{array} \right. \\
     *   \phi(\varpi) &=&
     *   \left\{
     *   \begin{array}{rl}
     *   \frac{1}{n\:\phi_0} I_0(m\:\sqrt{b^2-(2\:\pi\:k/n)^2}) & \mbox{for } k = -n\:(1-\frac{1}{2\sigma}), ..., n\:(1-\frac{1}{2\sigma})\\
    ...
    double kaiser_bessel_function<DIMENSION>::d_phi_t(double x)
    {
      ...
      if(val>1.e-12)
        result = 1./M_PI*(n*n*x)*((-b_val()*cosh(b_val()*sqrt(val)))/val + sinh(b_val()*sqrt(val))/( std::pow(val,3./2.) ));

      if(val<-1.e-12)
        result = 1./M_PI*(n*n*x)*((b_val()*cos(b_val()*sqrt(-val)))/(-val) - sin(b_val()*sqrt(-val))/( std::pow(-val,3./2.) ));
    ...
    double kaiser_bessel_function<DIMENSION>::phi_wn(int wn)
    {
      if(square(wn) < square(n*(1.-1./(2.*sigma))))
        return 1./double(n) * besselI0(m*sqrt( square(b_val()) - square((2.*M_PI*double(wn)/double(n))))) / renorm_factor();
      else
        throw std::logic_error(__FUNCTION__);
    }
