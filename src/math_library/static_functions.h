...
bool pair_less(std::pair<std::complex<double>, std::complex<double>> const& x,
...
bool susceptibility_less_pairs(std::pair<std::complex<double>, int> const& x,
                               std::pair<std::complex<double>, int> const& y) {
  return sqrt(square(real(x.first) - 1.) + square(imag(x.first))) >
         sqrt(square(real(y.first) - 1.) + square(imag(y.first)));
}

bool real_pair_less(std::pair<double, int> const& x, std::pair<double, int> const& y) {
