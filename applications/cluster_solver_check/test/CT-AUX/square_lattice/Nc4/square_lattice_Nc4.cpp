...
TEST(DCA_loop_sp, Self_Energy) {
  ...
  FUNC_LIB::function<std::complex<double>, dmn_4<nu, nu, k_DCA, w> > Sigma_ED(
      MOMS_imag.Sigma);
  ...
  // Compare ED self-energy with QMC self-energy
  if (dca_test_env->concurrency.id() == dca_test_env->concurrency.first()) {
    for (int k_ind = 0; k_ind < k_DCA::dmn_size(); ++k_ind) {
      double ED_min_QMC_inf = 0.;
      double ED_inf = 0.;
      for (int w_ind = 0; w_ind < w::dmn_size(); ++w_ind) {
        ED_min_QMC_inf = std::max(
            ED_min_QMC_inf, std::abs(Sigma_ED(0, 0, 0, 0, k_ind, w_ind) -
...
int main(int argc, char** argv) {
  ...
  dca_test_env =
      new dca_mpi_test_environment(argc, argv, "input.square_lattice_Nc4.json");
