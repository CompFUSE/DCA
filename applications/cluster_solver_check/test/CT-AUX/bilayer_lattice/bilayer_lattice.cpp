    ...
    for (int b2 = 0; b2 < b::dmn_size(); ++b2) {
      for (int k_ind = 0; k_ind < k_DCA::dmn_size(); ++k_ind) {
        ...
        for (int w_ind = 0; w_ind < w::dmn_size(); ++w_ind) {
          ED_min_QMC_inf = std::max(ED_min_QMC_inf, std::abs(Sigma_ED(0, 0, b2, 0, k_ind, w_ind)
                                                             - Sigma_QMC(0, 0, b2, 0, k_ind, w_ind)));
          ED_inf = std::max(ED_inf, std::abs(Sigma_ED(0, 0, b2, 0, k_ind, w_ind)));
        }
        EXPECT_LT(ED_min_QMC_inf, 2.e-2);
        if (b2 == 0) {
          EXPECT_LT(ED_min_QMC_inf/ED_inf, 1.e-1);
        }
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  dca_test_env = new dca_mpi_test_environment(
      argc, argv, "input.bilayer_lattice.json");
